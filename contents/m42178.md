...
{0} $$
</sub>, $$ x_{0} $$, and $$ t_{0} $$ are initial values), and the average
angular velocity $$ \overline{\omega } $$ and average velocity $$
...
<div data-type="equation" id="eip-74">
 $$ \omega =0+\left( 110 {\text{rad/s}}^{2}\right)\left(2.00\text{s}\right)
=220\text{rad/s}\text{.} $$ 
</div>
...
<div data-type="equation" id="eip-147">
 $$ v=\left(0.0450 \text{m}\right)\left(220 \text{rad/s}\right)=9.90 \text
{m/s.} $$ 
</div>
...
rev}=\text{2π rad} $$, we can find the number of revolutions by finding $$
\theta $$ in radians. We are given $$ \alpha $$ and $$ t $$, and we know $$
\omega_{0} $$ is zero, so that $$ \theta $$ can be obtained using $$ \theta
=\omega_{0} t+\frac{1}{2} \alpha t^{2} $$.

<div data-type="equation" id="eip-252">
 $$ \begin{array}{lll}\theta & =& \omega_{0} t+\frac{1}{2}\alpha t^{2}\\ & =& 0+\left(0.500\right)\left(110{\text{rad/s}}^{2}\right){\left(2.00 \text{s}\right)}^{2}=220 \text{rad}.\end{array} $$ 
</div>
Converting radians to revolutions gives

<div data-type="equation" id="eip-689">
 $$ \theta =\left(220 \text{rad}\right)\frac{1 \text{rev}}{\text{2π rad}}
=35.0 \text{rev.} $$ 
</div>
**Solution for (d)**
...
<div data-type="equation" id="eip-665">
 $$ x=\mathrm{r\theta }=\left(0.0450 \text{m}\right)\left(220 \text{rad}\right)=9.90 \text{m}. $$ 
</div>
**Discussion**
...
velocity is $$ \omega_{0}=220 \text{rad/s} $$ and the final angular velocity $$
\omega $$ is zero. The angular acceleration is given to be $$ \alpha =-300
\text{rad/s}^{2} $$. Examining the available equations, we see all quantities
...
<div data-type="equation" id="eip-309">
 $$ t=\frac{\omega -\omega_{0} }{\alpha }=\frac{0-220 \text{rad/s}}
{-300{\text{rad/s}}^{2}}=0.733 \text{s.} $$ 
</div>
**Discussion**
...
Large freight trains accelerate very slowly. Suppose one such train accelerates from rest, giving its 0.350-m-radius wheels an angular acceleration of  $$ 0.250{\text{rad/s}}^{2} $$ . After the wheels have made 200 revolutions (assume no slippage): (a) How far
...
<div data-type="equation" id="eip-271">
 $$ x= r\theta =\left(0.350 m\right)\left(1257 \text{rad}\right)=440 \text{m}. $$ 
</div>
**Solution for (b)**
...
<div data-type="equation" id="eip-6">
 $$ \begin{array}{lll}
\omega & =& {\left[ 0+ 2\left(0.250 \text{rad/s}^{2} \right)
...
<div data-type="equation" id="eip-84">
 $$ v=r \omega =\left(0.350 m\right)\left(25.1 \text{rad/s}\right)=8.77 \text{m/s}. $$ 
</div>
**Discussion**
...
<div data-type="equation" id="eip-624">
 $$ x=r \theta =\left(0.15\text{m}\right)\left(75.4\text{rad}\right)
=11\text{m}. $$ 
</div>
**Discussion**
...
* {: #import-auto-id1374592} In these equations, the subscript 0 denotes initial
  ...
  \overline{\omega } $$ and average velocity $$ \overline{v} $$ are
  defined as follows:
