...
Since elapsed time is $$ \Delta t=t_{f}-t_{0} $$, taking $$ t_{0}=0 $$
means that $$ \Delta t=t_{f} $$, the final time on the stopwatch. When initial time is taken to be zero, we use
the subscript 0 to denote initial values of position and velocity. That is, $$
x_{0} $$
*is the initial position* and $$ v_{0} $$
...
x=x-x_{0} $$. Also, it simplifies the expression for change in velocity, which is now $$
\Delta v=v-v_{0} $$. To summarize, using the simplified notation, with the initial time taken to be
zero,

<div data-type="equation" id="import-auto-id2563693">
 $$ \begin{array}{lll}\Delta t& =& t\\ \Delta x& =& x-x_{0}\\ \Delta v& =& v-v_{0}\end{array}} $$ 
</div>
...
<div data-type="equation" id="import-auto-id1782525">
 $$ \overline{v}=\frac{\Delta x}{\Delta t}\text{.} $$ 
</div>
...
<div data-type="equation" id="import-auto-id2297251">
 $$ x=x_{0}+\overline{v}t\text{,} $$ 
</div>
where the average velocity is

<div data-type="equation" id="import-auto-id2086556">
 $$ \overline{v}=\frac{ v_{0}+v}{2}\left(\text{constant}a\right)\text{.} $$ 
</div>
</div>

The equation $$ \overline{v}=\frac{ v_{0}+v}{2} $$
...
<div data-type="equation" id="import-auto-id4152098">
 $$ \overline{v}=\frac{ v_{0}+v}{2}=\frac{30 \text{km/h}+60 \text{km/h}}{2}=\text{45 km/h,} $$ 
</div>
...
, we identify the values of $$ x_{0} $$, * $$ \overline{v} $$
*, and $$ t $$
** from the statement of the problem and substitute them into the equation.

**Solution**

1\. Identify the knowns. $$ \overline{v}=4.00 \text{m/s} $$, $$ \Delta t=2.00 \text{min} $$, and $$ x_{0}=0 \text{m} $$.**
...
The equation $$ x=x_{0}+\overline{v}t $$
...
<div data-type="equation" id="import-auto-id1962571">
 $$ a=\frac{v-v_{0}}{t}\text{}\left(\text{constant}a\right)\text{.} $$ 
</div>
Solving for  $$ v $$ 
...
<div data-type="equation" id="import-auto-id2168495">
 $$ v=v_{0}+a t  \text{}\left(\text{constant}a\right)\text{.} $$ 
</div>
</div>
...
1\. Identify the knowns. $$ v_{0}=70.0 \text{m/s} $$, $$ a=-1.50 \text{m/s}^2 $$, $$ t=40.0\text{s} $$.
...
In addition to being useful in problem solving, the equation $$ v=v_
{0}+a t   $$
...
<div data-type="equation" id="import-auto-id1689620">
 $$ \frac{ v_{0}+v}{2}=v_{0}+\frac{1}{2}a t  \text{.} $$ 
</div>
Since  $$ \frac{ v_{0}+v}{2}=\overline{v} $$ 

for constant acceleration, then

<div data-type="equation" id="import-auto-id2301379">
 $$ \overline{v}=v_{0}+\frac{1}{2}a t  \text{.} $$ 
</div>
Now we substitute this expression for  $$ \overline{v} $$ 

into the equation for displacement, $$ x=x_{0}+\overline{v}t $$, yielding

<div data-type="equation" id="import-auto-id1807031">
 $$ x=x_{0}+v_{0}t+\frac{1}{2}a t^{2} \left(\text{constant}a\right)\text{.} $$ 
</div>
</div>
...
can use the equation $$ x=x_{0}+v_{0}t+\frac{1}{2}a t^{2}  $$
**once we identify $$ v_{0} $$, $$ a $$, and $$ t $$
...
<div data-type="equation" id="import-auto-id2171407">
 $$ x=x_{0}+v_{0}t+\frac{1}{2}a t^{2} \text{.} $$ 
</div>
...
What else can we learn by examining the equation $$ x=x_{0}+v_
{0}t+\frac{1}{2}a t^{2} ? $$
We see that:
...
* {: #import-auto-id2167534} if acceleration is zero, then the initial velocity
  ...
  $$ v_{0}=\overline{v} $$
  ) and $$ x=x_{0}+v_{0}t+\frac{1}{2}a t^{2}  $$
  becomes $$ x=x_{0}+v_{0}t $$
<div data-type="note" data-has-label="true" data-label="" markdown="1">
<div data-type="title">
...
If we solve $$ v=v_{0}+a t   $$
for $$ t $$, we get

<div data-type="equation" id="import-auto-id2365760">
 $$ t=\frac{v-v_{0}}{a}\text{.} $$ 
</div>
Substituting this and  $$ \overline{v}=\frac{ v_{0}+v}{2} $$ 

into $$ x=x_{0}+\overline{v}t $$, we get

<div data-type="equation" id="import-auto-id1659227">
 $$ {v}^{2}=v_{0}^{2}+2a\left(x-x_{0}\right)\text{}\left(\text{constant}a\right)\text{.} $$ 
</div>
</div>
...
The equation $$ {v}^{2}=v_{0}^{2}+2a\left(x-x_{0}\right) $$
...
1\. Identify the known values. We know that $$ v_{0}=0 $$, since the dragster starts from rest. Then we note that $$ x-x_{0}=\text{402
m} $$
...
; $$ a=-7.00\text{m/s}^{2} $$
( $$ a $$
is negative because it is in a direction opposite to velocity). We take $$ x_
{0} $$
to be 0. We are looking for displacement $$ \Delta x $$, or $$ x-x_{0} $$.
...
<div data-type="equation" id="import-auto-id2178635">
 $$ x_{\text{wet}}=90.\text{0 m on wet concrete}\text{.} $$ 
</div>
**Solution for (c)**
...
\overline{v}=30.0 \text{m/s} $$
; $$ t_{\text{reaction}}=0.500\text{s} $$
; $$ a_{\text{reaction}}=0 $$. We take $$ x_{0-\text{reaction}} $$
to be 0. We are looking for $$ x_{\text{reaction}} $$.
...
2\. We need to solve for $$ t $$. Choose the best equation. $$ x=x_{0}+v_{0}t+\frac{1}{2}a t^{2} 
$$
...
We have been using SI units of meters per second squared to describe some examples of acceleration or deceleration of cars, runners, and trains. To achieve a better feel for these numbers, one can measure the braking deceleration of a car doing a slow (and safe) stop. Recall that, for average acceleration,  $$ \overline{a}=\Delta v/\Delta t $$ . While traveling in a car, slowly apply the brakes as you come up to a stop
...
<div data-type="equation" id="import-auto-id2168930">
 $$ t=\frac{v-v{}_{0}\text{}}{a}=\frac{400 \text{m/s}-0 \text{m/s}}{ 20 \text{m/s}^2}=20 \text{s} $$ 
</div>
</div>
</div>
...
<div data-type="problem" markdown="1">
An Olympic-class sprinter starts a race with an acceleration of  $$ 4.50 \text{m/s}^2 $$ . (a) What is her speed 2.40 s later? (b) Sketch a graph of her position vs.
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) A light-rail commuter train accelerates at a rate of  $$ 1.35 \text{m/s}^2 $$ . How long does it take to reach its top speed of 80.0 km/h, starting from
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
At the end of a race, a runner decelerates from a velocity of 9.00 m/s at a rate of  $$ 2.00 \text{m/s}^2 $$ . (a) How far does she travel in the next 5.00 s? (b) What is her final
...
<sup /> and in multiples of  $$ g\left(g=9.80\text{m/s}^{2}\right) $$ . (b) Calculate the stopping time. (c) The tendons cradling the brain stretch,
...
(b)  $$ 162 \text{m/s} $$
(c)  $$ v>v_{\text{max}} $$, because the assumption of constant acceleration is not valid for a dragster. A
