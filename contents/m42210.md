...
<div data-type="equation" id="import-auto-id2447591">
 $$\begin{array}{lll}{N}_{\text{R}}& =& \frac{2\rho \text{vr}}{\eta }\\ & =& \frac{2\left({1025 \text{kg/m}}^{3}\right)\left(\text{1.70 m/s}\right)\left( 0.150 \times 10^{-3}  \text{m}\right)}{ 1.00 \times 10^{-3}  \text{N}\cdot {\text{s/m}}^{2}}\\ & =& \text{523}\text{.}\end{array} $$
</div>
**Discussion**
...
* of air traveling through the trachea during each inhalation. Now calculate the average air speed in meters per second through your trachea during each inhalation. The radius of the trachea in adult humans is approximately  $$10^{-2} \text{m} $$
