...
Now  $$\Delta \left(B \text{cos} \theta \right)=0.200 \text{T} $$
, since it was given that  $$B \text{cos} \theta  $$
 changes from 0.0500 to 0.250 T. The area of the loop is  $$A={\mathrm{\pi r}}^{2}=\left( 3.14\text{.}\text{.}\text{.}\right)\left(0.060 \text{m}{\right)}^{2}= 1.13 \times 10^{-2}  {\text{m}}^{2} $$
. Thus,
...
<div data-type="equation" id="eip-984">
 $$\text{Emf}=N\frac{\Delta \Phi }{\Delta t}=\frac{\left( 1.13 \times 10^{-2}  {\text{m}}^{2}\right)\left( 0.200 \text{T}\right)}{ 0.100 \text{s}}= 22.6 \text{mV.} $$
</div>
**Discussion**
