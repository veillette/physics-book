...
concepts discussed in [Thermodynamics](/ch15Thermodynamics) it is then possible to calculate
how long it would take for the surface to cool to rock-formation temperatures.
The result is about $$10^{9} $$ years. The first rocks formed have been solid
for $$ 3.5 \times 10^{9} $$ years, so that the age of the Earth is approximately
$$ 4.5 \times 10^{9} $$ years. There is a large body of other types of
evidence (both Earth-bound and solar system characteristics are used) that
supports this age. The puzzle is that, given its age and initial temperature,
the center of the Earth should be much cooler than it is today (
