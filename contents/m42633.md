...
Some nuclides are stable, apparently living forever. Unstable nuclides decay (that is, they are radioactive), eventually producing a stable nuclide after many decays. We call the original nuclide the **parent** and its decay products the **daughters**. Some radioactive nuclides decay in a single step to a stable nucleus. For example,  $${}^{60}\text{Co} $$
 ...
 and  $${}^{210}\text{Po} $$ ,
 which the Curies first discovered (see [[Figure 1]](#Figure1)). Radon gas is also produced ( $${}^{222}\text{Rn} $$
...
Note that the daughters of  $$\alpha  $$
 decay shown in [[Figure 1]](#Figure1) always have two fewer protons and two fewer neutrons than the parent. This seems reasonable, since we know that  $$\alpha  $$
...
In **alpha decay**{: class="term"}, a  $${}^{4}\text{He} $$
 nucleus simply breaks away from the parent nucleus, leaving a daughter with two fewer protons and two fewer neutrons than the parent (see [[Figure 2]](#Figure2)). One example of  $$\alpha  $$
 decay is shown in [[Figure 1]](#Figure1) for  $${}^{238}\text{U} $$ .
...
where Y is the nuclide having one more proton than X (see [[Figure 4]](#Figure4)). So if you know that a certain nuclide  $${\beta }^{-} $$
...
decay. Certain nuclides decay by the emission of a *positive* electron. This is **antielectron** or **positron decay** (see [[Figure 5]](#Figure5)).
...
These are called cobalt  $$\gamma  $$
 ...
 decay does not change the nuclide to another species, it is not prominently featured in charts of decay series, such as that in [[Figure 1]](#Figure1).
