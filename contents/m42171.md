...
The two children shown in [\[link\]](#import-auto-id3083545) are balanced on a seesaw of negligible mass. (This assumption is made to keep the example simple—more involved examples will follow.) The first child has a mass of 26.0 kg and sits 1.60 m from the pivot.(a) If the second child has a mass of 32.0 kg, how far is she from the pivot? (b) What is  $$ {F}_{\text{p}} $$ 
...
Note that a minus sign has been inserted into the second equation because this torque is clockwise and is therefore negative by convention. Since  $$ {F}_{\text{p}} $$ 
...
This part asks for a force $$ {F}_{\text{p}} $$. The easiest way to find it is to use the first condition for equilibrium,
which is
...
not an approximation—the distances $$ {r}_{\text{1}} $$
and $$ {r}_{\text{2}} $$
...
<div data-type="equation" id="import-auto-id1404316">
 $$ \begin{array}{lll}{F}_{\text{p}}& =& \left(\text{26.0 kg}+\text{32.0 kg}+\text{12.0 kg}\right)\left(\text{9.80}\text{m}/{\text{s}}^{2}\right)\\ & =& \text{686 N}\end{array} $$ 
</div>
</div>
</div>
