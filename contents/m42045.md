...
{v}_{\text{tot}}=\sqrt{ v_{x}^{2}+v_{y}^{2}} $$
and $$ \theta ={\tan}^{-1}\left(v_{y}/v_{x}\right) $$
directly.
...
<div data-type="equation" id="eip-120">
 $$ {v}_{\text{tot}}=\sqrt{ v_{x}^{2}+v_{y}^{2}}\text{,} $$ 
</div>
where

<div data-type="equation" id="eip-845">
 $$ v_{x}={v}_{\text{river}}=1.20 \text{m/s} $$ 
</div>
and

<div data-type="equation" id="eip-311">
 $$ v_{y}={v}_{\text{boat}}=0\text{.}\text{750 m/s.} $$ 
</div>
Thus,
...
<div data-type="equation" id="eip-862">
 $$ \theta ={\tan}^{-1}\left(v_{y}/v_{x}\right)={\tan}^{-1}\left(0.750/1.20\right)\text{.} $$ 
</div>
This equation gives
...
due east and flies with a strong tailwind. It travels 3000 km in a direction $$ 5º $$
