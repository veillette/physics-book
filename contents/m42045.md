...
In each of these situations, an object has a **velocity**{: data-type="term"
#import-auto-id1831655} relative to a medium (such as a river) and that medium
...
. The velocity of the boat, \\( {\mathbf{\text{v}}}_{\text{boat}} \\)

, is 0.75 m/s in the \\( y \\)

-direction relative to the river and the velocity of the river, \\(
{\mathbf{\text{v}}}_{\text{river}} \\)
...
is known, and we are asked to find \\( {\mathbf{\text{v}}}_{\text{w}} \\)

. None of the velocities are perpendicular, but it is possible to find their
components along a common set of perpendicular axes. If we can find the
components of \\( {\mathbf{\text{v}}}_{\text{w}} \\)

, then we can combine them to solve for its magnitude and direction. As shown
...
# import-auto-id1447237} is limited to situations where speeds are less than about 1% of the speed of light—that is, less
...
{y}=-5.42\phantom{\rule{0.25em}{0ex}}\text{m/s} \\)

, the same as found in part (a). In contrast to part (a), there now is a
...
. The *x*- and *y*-components of velocity can be combined to find the magnitude
...
; rather, it is \\( \text{260}\text{.}\text{06 m/s} \\)

. The velocity’s magnitude had to be calculated to five digits to see any
