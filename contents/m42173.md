...
net}\phantom{\rule{0.25em}{0ex}}F=0\right) \\)

. The second condition \\( \text{(net}\phantom{\rule{0.25em}{0ex}}{\tau }_
{}=\text{0)} \\)
...
weight of the pole, \\( {F}_{R}={F}_{L}=w/2 \\)
...
, the force exerted by the right hand, and (b)  \\( {F}_{L} \\)

, the force exerted by the left hand. The hands are 0.900 m apart, and the cg of
...
** is seen to be exactly half of \\( {F}_{R} \\)

, as we might have guessed, since \\( {F}_{L} \\)
