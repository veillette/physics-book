...
use the notation that a boldface symbol, such as \\( \text{D} \\)

, stands for a vector. Its magnitude is represented by the symbol in italics,
\\( D \\)

, and its direction by \\( \theta \\)
...
represented by a variable in italics, such as \\( F \\)
...
The **tail**{: data-type="term"

# import-auto-id1165298535408} of the vector is the starting point of the vector, and the **
head**{: data-type="term"

# import-auto-id1165298982372} (or tip) of a vector is the final, pointed end of the arrow.
...
<strong><em>Step 6. </em></strong>To get the **direction**{: data-type="term"
...
, making the lengths proportional to the distance and the directions as
...
(4) Use a ruler to measure the magnitude of \\( \mathbf{\text{R}} \\)

, and a protractor to measure the direction of \\( \text{R} \\)

. While the direction of the vector can be specified in many ways, the easiest
...
has the same length as \\( \mathbf{\text{–B}} \\)

, but points in the opposite direction. Essentially, we just flip the vector so
...
, and the second leg of the trip with a vector \\( \mathbf{\text{B}} \\)

. The dock is located at a location \\( \mathbf{\text{A}}+\mathbf{\text{B}} \\)

. If the woman mistakenly travels in the *opposite* direction for the second leg
of the journey, she will travel a distance \\( B \\)

(30.0 m) in the direction \\( 180º–112º=68º \\)

south of east. We represent this as \\( \mathbf{\text{–B}} \\)

, as shown below. The vector \\( \mathbf{\text{–B}} \\)
...
, with the location at which the woman mistakenly arrives, \\( \text{A + }\left(
\text{–B}\right) \\)
...
. We obtain the resultant vector \\( \mathbf{\text{R}}\text{'} \\)
...
, or 82.5 m, in a direction \\( \text{66}\text{.}0\text{º} \\)

north of east. This is an example of multiplying a vector by a positive **
scalar**{: data-type="term"

# import-auto-id1165296219603}. Notice that the magnitude changes, but the direction stays the same.
...
. Vectors are multiplied by scalars in many situations. Note that division is
...
, as in [[link]](#import-auto-id1165296241785), then this problem asks you to
find their sum \\(
...
, as in [[link]](#import-auto-id1165296430663), then this problem finds their
sum \\( \text{R = A + B} \\)
...
south of west, and then leg \\( \mathbf{A} \\)

, which is 12.0 m in a direction exactly \\( \text{20º} \\)
...
, all having different lengths and directions. Find the sum \\( \text{A + B + C}
\\)
