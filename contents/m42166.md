...
fact: (a) Calculate the increase in velocity of a 20 000-kg space probe that
expels only 40.0-kg of its mass at the given exhaust velocity. (b)
...
same as on Earth’s surface $$ \left(
9.80\text{m/s}^{2}\right) $$. (b) Why might it be necessary to limit the
acceleration of a rocket?
...
neglected. (c) What is unreasonable about the results? (d) Which premise is
