...
exhaust velocity of the gases relative to the rocket, $$ v_
{\text{e}} $$, the greater the acceleration is. The practical limit for ** $$ v_{\text{e}}
$$
...
$$ 11.2\times 10^{3}\text{m/s} $$, and assuming an exhaust velocity $$ v_
{\text{e}}=2.5\times 10^{3}\text{m/s} $$.
...
acting on the rocket, so$$ F_
{\text{thrust}}=v_{\text{e}}\frac{\Delta m}{\Delta t} $$, where all quantities are positive. Applying Newton’s second law to the rocket
gives$$ F_
