...
Intensity is defined to be the power per unit area carried by a wave. Power is the rate at which energy is transferred by the wave. In equation form, **intensity**{: data-type="term" #import-auto-id3062860} *$$I$$
* is
...
$$I=\frac{ {\left(\Delta p\right)}^{2}}{2{\mathrm{\rho v}}_{w}}.$$
</div>
...
. (We are using a lower case $$p$$
 ...
 above.) The energy (as kinetic energy $$\frac{ {\mathrm{mv}}^{2}}{2}$$
...
where $${I}_{0}={\text{10}}^{\text{–12}}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
...
* is defined in terms of a ratio, it is a unitless quantity telling you the *level* of the sound relative to a fixed standard ($${\text{10}}^{\text{–12}}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
...
The decibel level of a sound having the threshold intensity of $${\text{10}}^{–\text{12}}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
 is $$\beta =0\phantom{\rule{0.25em}{0ex}}\text{dB}$$
, because $${\text{log}}_{\text{10}}1=0$$
...
, so that only $${\text{10}}^{–\text{16}}$$
 W falls on it at the threshold of hearing! Air molecules in a sound wave of this intensity vibrate over a distance of less than one molecular diameter, and the gauge pressures involved are less than $${\text{10}}^{–9}$$
 atm.

Another impressive feature of the sounds in [\[link\]](#import-auto-id3250463) is their numerical range. Sound intensity varies by a factor of $${\text{10}}^{\text{12}}$$
...
One more observation readily verified by examining [\[link\]](#import-auto-id3250463) or using $$I={\frac{\left(\Delta p\right)}{2{\text{ρv}}_{\text{w}}}}^{2}$$
 is that each factor of 10 in intensity corresponds to 10 dB. For example, a 90 dB sound compared with a 60 dB sound is 30 dB greater, or three factors of 10 (that is, $${\text{10}}^{3}$$
 times) as intense. Another example is that if one sound is $${\text{10}}^{7}$$
...
, we can calculate $$\beta $$
 straight from its definition in $$\beta \phantom{\rule{0.25em}{0ex}}\left(\text{dB}\right)=\text{10}\phantom{\rule{0.25em}{0ex}}{\text{log}}_{\text{10}}\left(I/{I}_{0}\right)$$
.
...
Air has a density of $$1.29 kg{/m}^{3}$$
...
$$I=\frac{ {\left(\Delta p\right)}^{2}}{2{\text{ρv}}_{w}}=\frac{ {\left(0.656 Pa\right)}^{2}}{2\left(1\text{.}\text{29}\phantom{\rule{0.25em}{0ex}}{\text{kg/m}}^{3}\right)\left(\text{331 m/s}\right)}=5\text{.}\text{04}×{\text{10}}^{-4}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}.$$
</div>
(3) Enter the value for $$I$$
 and the known value for $${I}_{0}$$
 into $$\beta \phantom{\rule{0.25em}{0ex}}\left(\text{dB}\right)=\text{10}\phantom{\rule{0.25em}{0ex}}{\text{log}}_{\text{10}}\left(I/{I}_{0}\right)$$
. Calculate to find the sound intensity level in decibels:

<div data-type="equation" id="eip-273">
$${10 log}_{10}\left(5.04×{10}^{8}\right)=10\phantom{\rule{0.25em}{0ex}}\left(8.70\right)\phantom{\rule{0.25em}{0ex}}dB=87 dB.$$
</div>
**Discussion**
...
$${\beta }_{2}-{\beta }_{1}=3\phantom{\rule{0.25em}{0ex}}\text{dB}.$$
</div>
Note that:

<div data-type="equation" id="eip-281">
$${\text{log}}_{\text{10}}b-{\text{log}}_{\text{10}}a={\text{log}}_{\text{10}}\left(\frac{b}{a}\right)\text{.}$$
</div>
(2) Use the definition of $$\beta $$
 to get:

<div data-type="equation" id="eip-123">
$${\beta }_{2}-{\beta }_{1}={\text{10 log}}_{\text{10}}\left(\frac{ {I}_{2}}{ {I}_{1}}\right)=\text{10}\phantom{\rule{0.25em}{0ex}}{\text{log}}_{\text{10}}2.00=\text{10}\phantom{\rule{0.25em}{0ex}}\left(0\text{.}\text{301}\right)\phantom{\rule{0.25em}{0ex}}\text{dB}.$$
</div>
Thus,

<div data-type="equation" id="eip-410">
$${\beta }_{2}-{\beta }_{1}=3\text{.01 dB}.$$
</div>
**Discussion**
...
* {: #import-auto-id1448826} Sound intensity level in units of decibels (dB) is
  ...
  where $${I}_{0}={10}^{–12}\phantom{\rule{0.25em}{0ex}}W/{\text{m}}^{2}$$
...
$$3.16×{10}^{–4}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
</div>
</div>
</div>
...
$$3.04×{10}^{–4}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
</div>
</div>
</div>
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
What sound intensity level in dB is produced by earphones that create an intensity of $$4.00×{\text{10}}^{\text{−2}}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{\text{2}}$$
?
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Show that an intensity of $${10}^{–12}\phantom{\rule{0.25em}{0ex}}{W/m}^{2}$$
 is the same as $${10}^{–16}\phantom{\rule{0.25em}{0ex}}{W/cm}^{2}$$
.
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) What is the intensity of a sound that has a level 7.00 dB lower than a $$4.00×{10}^{–9}\phantom{\rule{0.25em}{0ex}}{W/m}^{2}$$
 sound? (b) What is the intensity of a sound that is 3.00 dB higher than a $$4.00×{10}^{–9}\phantom{\rule{0.25em}{0ex}}{W/m}^{2}$$
 sound?
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
If a sound intensity level of 0 dB at 1000 Hz corresponds to a maximum gauge pressure (sound amplitude) of $${10}^{–9}\phantom{\rule{0.25em}{0ex}}\text{atm}$$
...
$$1.45×{10}^{–3}\phantom{\rule{0.25em}{0ex}}\text{J}$$
</div>
</div>
</div>
