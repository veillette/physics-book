...
<div data-type="equation" id="eip-969">
 $$I=\frac{ {\left(\Delta p\right)}^{2}}{2{\text{ρv}}_{w}}=\frac{ {\left(0.656 Pa\right)}^{2}}{2\left(1\text{.}\text{29} {\text{kg/m}}^{3}\right)\left(\text{331 m/s}\right)}= 5.04 \times 10^{-4}  {\text{W/m}}^{2}. $$
</div>
