...
nonconservative force**{: data-type="term"

# import-auto-id2125883} is one for which work depends on the path taken. Friction is a good example of a nonconservative
...
its kinetic energy, or \\( {W}_{\text{net}}=\text{ΔKE} \\)

. The net work is the sum of the work by nonconservative forces plus the work by
...
energy, so that \\( {W}_{\text{c}}=-\text{Δ}\text{PE} \\)

. Substituting this equation into the previous one and solving for \\( {W}_
{\text{nc}} \\)
...
along the hill, with \\(
h=d\phantom{\rule{0.25em}{0ex}}\text{sin}\phantom{\rule{0.25em}{0ex}}5.00º \\)
...
cup, which moves horizontally. The work done by friction is \\( \text{fd} \\)
