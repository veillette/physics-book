...
<div data-type="equation" id="import-auto-id2578506">
 $$\begin{array}{lll}t& =& \frac{ {x}_{\text{rms}}^{2}}{2D}=\frac{\left(\text{0.010 m}{\right)}^{2}}{2\left(6\text{.}7×{\text{10}}^{-\text{10}} {\text{m}}^{2}\text{/s}\right)}\\ & =&  7.5 \times 10^{4}  \text{s}=\text{21 h}\text{.}\end{array} $$
</div>
**Discussion**
