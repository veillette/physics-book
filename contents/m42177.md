...
[Uniform Circular Motion and Gravitation](/contents/m42140) discussed only uniform circular motion, which is motion in a circle at constant speed and, hence, constant angular velocity. Recall that angular velocity  \\( \omega  \\) 
...
where  \\( \theta  \\) 
 ...
 is the angle of rotation as seen in [\[link\]](#import-auto-id1941476). The relationship between angular velocity  \\( \omega  \\) 
...
Angular velocity is not constant when a skater pulls in her arms, when a child starts up a merry-go-round from rest, or when a computer’s hard disk slows to a halt when switched off. In all these cases, there is an **angular acceleration**{: data-type="term" #import-auto-id2438108}, in which  \\( \omega  \\) 

 changes. The faster the change occurs, the greater the angular acceleration. Angular acceleration  \\( \alpha  \\) 
...
where  \\( \Delta \omega  \\) 
 ...
 is the **change in angular velocity**{: data-type="term" #import-auto-id3406201} and  \\( \Delta t \\) 

 is the change in time. The units of angular acceleration are  \\( \left(\text{rad/s}\right)\text{/s} \\) 
...
. (b) If she now slams on the brakes, causing an angular acceleration of  \\( –87.3\phantom{\rule{0.25em}{0ex}}{\text{rad/s}}^{2} \\) 

, how long does it take the wheel to stop?

**Strategy for (a)**

The angular acceleration can be found directly from its definition in  \\( \alpha =\frac{\Delta \omega }{\Delta t} \\) 

 because the final angular velocity and time are given. We see that  \\( \Delta \omega  \\) 
...
Because  \\( \Delta \omega  \\) 
 ...
 is in revolutions per minute (rpm) and we want the standard units of  \\( {\text{rad/s}}^{2} \\) 
...
In this part, we know the angular acceleration and the initial angular velocity. We can find the stoppage time by using the definition of angular acceleration and solving for  \\( \Delta t \\) 
...
Linear or tangential acceleration refers to changes in the magnitude of velocity but not its direction. We know from [Uniform Circular Motion and Gravitation](/contents/m42140) that in circular motion centripetal acceleration,  \\( {a}_{\text{c}} \\) 

, refers to changes in the direction of the velocity but not its magnitude. An object undergoing circular motion experiences centripetal acceleration, as seen in [\[link\]](#import-auto-id1995872). Thus,  \\( {a}_{\text{t}} \\) 

 and  \\( {a}_{\text{c}} \\) 

 are perpendicular and independent of one another. Tangential acceleration  \\( {a}_{\text{t}} \\) 
...
Now we can find the exact relationship between linear acceleration  \\( {a}_{\text{t}} \\) 
...
For circular motion, note that  \\( v=\mathrm{r\omega } \\) 
...
The radius  \\( r \\) 
 ...
 is constant for circular motion, and so  \\( \text{Δ}\left(\mathrm{r\omega }\right)=r\left(\Delta \omega \right) \\) 
...
We are given information about the linear velocities of the motorcycle. Thus, we can find its linear acceleration  \\( {a}_{\text{t}} \\) 

. Then, the expression  \\( \alpha =\frac{ {a}_{\text{t}}}{r} \\) 
...
So far, we have defined three rotational quantities—  \\( \theta \mathrm{, }\omega  \\) 

, and  \\( \alpha  \\) 

. These quantities are analogous to the translational quantities  \\( x\mathrm{, }v \\) 
...
The magnitude of angular acceleration is  \\( \alpha  \\) 
 ...
 and its most common units are  \\( {\text{rad/s}}^{2} \\) 
...
* {: #import-auto-id1549399} The radius r is constant for circular motion, and so
   \\( \mathrm{\text{Δ}}\left(\mathrm{r\omega }\right)=r\Delta \omega  \\) 
...
An ultracentrifuge accelerates from rest to 100,000 rpm in 2.00 min. (a) What is its angular acceleration in  \\( {\text{rad/s}}^{2} \\) 

? (b) What is the tangential acceleration of a point 9.50 cm from the axis of rotation? (c) What is the radial acceleration in  \\( {\text{m/s}}^{2} \\) 
...
You are told that a basketball player spins the ball with an angular acceleration of  \\( \text{100}{\text{ rad/s}}^{2} \\) 
