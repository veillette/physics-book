...
<div data-type="equation" id="eip-715">
 $$R=1\text{.}\text{097}×{\text{10}}^{7}/\text{m} \left({\text{or m}}^{-1}\right). $$
</div>
...
<div data-type="equation" id="eip-873">
 $$\begin{array}{lll}\frac{1}{\lambda }& =& R\left(\frac{1}{ {n}_{f}^{2}}-\frac{1}{ {n}_{i}^{2}}\right)\\ & =& \left(\text{1.097}×{\text{10}}^{7} {\text{m}}^{–1}\right)\left(\frac{1}{ {2}^{2}}-\frac{1}{ {4}^{2}}\right)\\ & =& \text{2.057}×{\text{10}}^{6} {\text{m}}^{–1}\text{.}\end{array} $$
</div>
Inverting to find  $$\lambda  $$
 gives

<div data-type="equation" id="eip-776">
 $$\begin{array}{lll}\lambda & =& \frac{1}{\text{2.057}×{\text{10}}^{6} {\text{m}}^{–1}}=\text{486}×{\text{10}}^{-9} \text{m}\\ & =& \text{486 nm.}\end{array} $$
</div>
**Discussion for (a)**
...
<div data-type="equation" id="eip-735">
 $$d \text{sin} \theta =\mathrm{m\lambda }\text{,} $$
</div>
where  $$d $$
...
<div data-type="equation" id="eip-995">
 $$d=\frac{\left(1\right)\left(\text{486 nm}\right)}{\text{sin 15º}}=1.88×{\text{10}}^{-6} \text{m}. $$
</div>
**Discussion for (b)**
...
<div data-type="equation" id="eip-434">
 $$\Delta E=\text{hf}={E}_{i}-{E}_{f}. $$
</div>

Here,  $$\Delta E $$
 is the change in energy between the initial and final orbits, and  $$\text{hf} $$
...
<div data-type="equation" id="eip-932">
 $${a}_{\text{B}}=\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kq}}_{e}^{2}}=\text{0.529}×{\text{10}}^{-\text{10}} \text{m}. $$
</div>
...
, where * $$V $$
* is the potential due to the nucleus, which looks like a point charge. The nucleus has a positive charge * $${\text{Zq}}_{e} $$
* ; thus,  $$V={\text{kZq}}_{e}/{r}_{n} $$
, recalling an earlier equation for the potential due to a point charge. Since the electron’s charge is negative, we see that * $$\text{PE}=-{\text{kZq}}_{e}/{r}_{n} $$
*. Entering the expressions for  $$\text{KE} $$
 and  $$\text{PE} $$
, we find
...
<div data-type="equation" id="eip-673">
 $${E}_{n}=-\frac{ {Z}^{2}}{ {n}^{2}}{E}_{0}\left(n=\text{1, 2, 3, ...}\right) $$
</div>

for the orbital **energies of hydrogen-like atoms**. Here, * $${E}_{0} $$
* is the <strong>ground-state energy </strong> $$\left(n=1\right) $$
...
<div data-type="equation" id="eip-885">
 $${E}_{n}=-\frac{\text{13.6 eV}}{ {n}^{2}}\text{(}n=\text{1, 2, 3, ...).} $$
</div>
...
Substituting  $${E}_{n}=\left(–\text{13.6 eV}/{n}^{2}\right) $$
, we see that

<div data-type="equation" id="eip-742">
 $$\text{hf}=\left(\text{13.6 eV}\right)\left(\frac{1}{ {n}_{\text{f}}^{2}}-\frac{1}{ {n}_{\text{i}}^{2}}\right). $$
</div>

Dividing both sides of this equation by  $$\text{hc} $$
 gives an expression for  $$1/\lambda  $$
\:

<div data-type="equation" id="eip-49">
 $$\frac{\text{hf}}{\text{hc}}=\frac{f}{c}=\frac{1}{\lambda }=\frac{\left(\text{13.6 eV}\right)}{\text{hc}}\left(\frac{1}{ {n}_{\text{f}}^{2}}-\frac{1}{ {n}_{\text{i}}^{2}}\right). $$
</div>

It can be shown that

<div data-type="equation" id="eip-369">
 $$\left(\frac{\text{13.6 eV}}{\text{hc}}\right)=\frac{\left(\text{13.6} \text{eV}\right)\left(1.602×{\text{10}}^{\text{−19}} \text{J/eV}\right)}{\left(6.626×{\text{10}}^{\text{−34}} \text{J·s}\right)\left(2.998×{\text{10}}^{8} \text{m/s}\right)}=1.097×{\text{10}}^{7} {\text{m}}^{–1}=R $$
</div>
...
, where * $$n $$
* is a non-negative integer. A downward transition releases energy, and so  $${n}_{i} $$
...
* The planetary model of the atom pictures electrons orbiting the nucleus in the way that planets orbit the sun. Bohr used the planetary model to develop the first reasonable theory of hydrogen, the simplest atom. Atomic and molecular spectra are quantized, with hydrogen spectrum wavelengths given by the formula
  ...
  <div data-type="equation" id="eip-144">
   $$R=\text{1.097}×{\text{10}}^{7} {\text{m}}^{-1}\text{.} $$
  </div>
...
* Bohr proposed that the allowed orbits are circular and must have quantized orbital angular momentum given by
  ...
  <div data-type="equation" id="eip-897">
   $${a}_{\text{B}}=\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kq}}_{e}^{2}}=\text{0.529}×{\text{10}}^{-\text{10}} \text{m}\text{.} $$
  </div>
...
<div data-type="solution" markdown="1">
 ...
 $$\lambda =\left(\frac{m}{1.097×{\text{10}}^{7}}\right)\left[\frac{\left(2×1{\right)}^{2}}{ {2}^{2}-{1}^{2}}\right]=1\text{.}\text{22}×{\text{10}}^{-7} \text{m}=\text{122 nm} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Look up the values of the quantities in  $${a}_{\text{B}}=\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kq}}_{e}^{2}}{}^{} $$
, and verify that the Bohr radius  $${a}_{\text{B}} $$
 is  $$\text{0.529}×{\text{10}}^{-\text{10}} \text{m} $$
.

</div>
<div data-type="solution" markdown="1">
 $${a}_{\text{B}}=\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kZq}}_{e}^{2}}=\frac{\left(\text{6.626}×{\text{10}}^{-\text{34}} \text{J·s}{\right)}^{2}}{ {4\pi }^{2}\left(9.109×{\text{10}}^{-\text{31}} \text{kg}\right)\left(8.988×{\text{10}}^{9} \text{N}\text{·}{\text{m}}^{2}/{C}^{2}\right)\left(1\right)\left(1.602×{\text{10}}^{-\text{19}} \text{C}{\right)}^{2}}=\text{0.529}×{\text{10}}^{-\text{10}} \text{m} $$
</div>
</div>
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
A hydrogen atom in an excited state can be ionized with less energy than when it is in its ground state. What is * $$n $$
...
<div data-type="solution" markdown="1">
 $$\text{2.12}×{\text{10}}^{\text{–10}} \text{m} $$
</div>
</div>

<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Show that  $$\left(13.6 eV\right)/\text{hc}=\text{1.097}×{\text{10}}^{7} \text{m}=R $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
A wavelength of  $$4\text{.}\text{653 μm} $$
...
Verify Equations  $${r}_{n}=\frac{ {n}^{2}}{Z}{a}_{\text{B}} $$
 and  $${a}_{B}=\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kq}}_{e}^{2}}=\text{0.529}×{\text{10}}^{-\text{10}} \text{m} $$
...
<div data-type="solution" markdown="1">
 ...
 From the equation  $${m}_{e}{\text{vr}}_{n}=n\frac{h}{2\pi }\text{,} $$
 we can substitute for the velocity, giving:  $${r}_{n}=\frac{ {\text{kZq}}_{e}^{2}}{ {m}_{e}}\cdot \frac{ {4\pi }^{2}{m}_{e}^{2}{r}_{n}^{2}}{ {n}^{2}{h}^{2}} $$
 so that  $${r}_{n}=\frac{ {n}^{2}}{Z}\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kq}}_{e}^{2}}=\frac{ {n}^{2}}{Z}{a}_{\text{B}}, $$
 where  $${a}_{\text{B}}=\frac{ {h}^{2}}{ {4\pi }^{2}{m}_{e}{\text{kq}}_{e}^{2}} $$
.
...
: a physical constant related to the atomic spectra with an established value of
   $$1.097×{\text{10}}^{\text{7}} {\text{m}}^{\text{−1}} $$
...
: Bohr formula for energies of electron states in hydrogen-like atoms:
   $${E}_{n}=-\frac{ {Z}^{2}}{ {n}^{2}}{E}_{0}\left(n=\text{1, 2, 3,} \dots \right) $$
</div>
