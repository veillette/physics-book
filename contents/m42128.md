...
north-east, then it is true that the vectors $$ {\mathbf{A}}_{x}{\text{ + A}}_{y}\text{ = A} $$. However, it is **not** true that the sum of the magnitudes of the vectors is
also equal. That is,
...
Then $$ A=10.3 $$
blocks and $$ \theta =29.1^\circ $$, so that

<div data-type="equation" id="eip-id1646569">
 $$  A_{x}=A\cos{\theta} =\left(10.3 \text{blocks}\right)\left(\cos{29.1º}\right)=9.0 \text{blocks} $$ 
</div>

<div data-type="equation" id="eip-id2495034">
 $$  A_{y}=A\sin{\theta} =\left(10.3 \text{blocks}\right)\left(\sin{29.1º}\right)=5.0 \text{blocks}\text{.} $$ 
</div>
...
perpendicular axes</em>.* Use the equations $$  A_{x}=A\cos{\theta}
$$
and $$  A_{y}=A\sin{\theta}
$$
...
Add the vector  $$ \mathbf{A} $$ to the vector $$ \mathbf{B} $$
...
\mathbf{A} $$
and $$ \mathbf{B} $$
along the **x**- and **y**-axes. Note that $$ A=53.0 m\text{} $$, $$ {\theta }_{\text{A}}=20.0^\circ $$, $$ B=34.0 m\text{} $$, and $$ {\theta }_{\text{B}}=63.0^\circ $$. We find the **x**-components by using $$  A_{x}=A\cos{\theta}
$$, which gives
...
Suppose you add two vectors  $$ \mathbf{A} $$ and $$ \mathbf{B} $$. What relative direction between them produces the resultant with the greatest
...
<div data-type="exercise" data-element-type="conceptual-questions">
<div data-type="problem" markdown="1">
If the vectors  $$ \mathbf{A} $$ and $$ \mathbf{B} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Do [[link]](#eip-287) again using analytical techniques and change the second leg of the walk to  $$ 25.0 \text{m} $$ straight south. (This is equivalent to subtracting $$ \mathbf{B} $$
from $$ \mathbf{A} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
A new landowner has a triangular piece of flat land she wishes to fence. Starting at the west corner, she measures the first side to be 80.0 m long and the next to be 105 m. These sides are represented as displacement vectors  $$ \mathbf{A} $$ from $$ \mathbf{B} $$
