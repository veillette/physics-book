...
** and ** $$  A_{y} $$, its **x**- and **{::}**y**-components, we use the following relationships for
a right triangle.
...
<div data-type="equation" id="eip-id1646569">
 $$  A_{x}=A\cos{\theta} =\left(\text{10.3 blocks}\right)\left(\cos29.1º\right)=\text{9.0 blocks} $$ 
</div>

<div data-type="equation" id="eip-id2495034">
 $$  A_{y}=A\sin{\theta} =\left(\text{10.3 blocks}\right)\left(\sin29.1º\right)=\text{5.0 blocks}\text{.} $$ 
</div>
...
<div data-type="equation" id="eip-750">
 $$ \theta ={\tan}^{-1}\left( A_{y}/ A_{x}\right)\text{.} $$ 
</div>
...
and $$ \theta $$
to $$  A_{x} $$
and $$  A_{y} $$. Equations $$ A=\sqrt{ A_{x}^{2}+ A_{y}^{2}} $$
and $$ \theta ={\tan}^{\text{–1}}\left( A_{y}/ A_{x}\right) $$
...
and $$ \theta ={\tan}^{–1}\left( A_{y}/ A_{x}\right) $$. When you use the analytical method of vector addition, you can determine the
...
perpendicular axes</em>.* Use the equations $$  A_
{x}=A\cos{\theta}
$$
and $$  A_
{y}=A\sin{\theta}
...
components are $$  A_{x} $$, $$  A_{y} $$, $$ {B}_{x} $$, and $$ {B}_{y} $$. The angles that vectors $$ \mathbf{A} $$
and $$ \mathbf{B} $$
...
$$
represents the first leg of a walk in which a person walks $$ 53.0 \text{m} $$
in a direction $$ 20.0\text{º} $$
north of east. Vector $$ \mathbf{B} $$
represents the second leg, a displacement of $$ 34.\text{0 m} $$
in a direction $$ 63.0\text{º} $$
north of east.
...
\mathbf{A} $$
and $$ \mathbf{B} $$
along the **x**- and **y**-axes. Note that $$ A=53.0 m\text{} $$, $$ {\theta }_{\text{A}}=20.0º $$, $$ B=34.0 m\text{} $$, and $$ {\theta }_{\text{B}}=63.0º $$. We find the **x**-components by using $$  A_
{x}=A\cos{\theta}
$$, which gives

<div data-type="equation" id="eip-253">
 $$ \begin{array}{lll} A_{x}& =& A\cos{\theta }_{A}=\left(53.0 m\text{}\right)\left(\text{cos 20.0º}\right)\\ & =& \left(53.0 m\right)\left(0\text{.940}\right)=49.8 m\end{array} $$ 
</div>
and

<div data-type="equation" id="eip-356">
 $$ \begin{array}{lll}{B}_{x}& =& B\cos{\theta }_{B}=\left(34.0 m\text{}\right)\left(\text{cos 63.0º}\right)\\ & =& \text{}\left(34.0 m\text{}\right)\left(0.454\right)=15.4 m\text{.}\end{array} $$ 
</div>
Similarly, the *y*-components are found using  $$  A_{y}=A\sin{\theta }_{A} $$ 

\:

<div data-type="equation" id="eip-802">
 $$ \begin{array}{lll} A_{y}& =& A\sin{\theta }_{A}=\left(53.0 m\text{}\right)\left(\text{sin 20.0º}\right)\\ & =& \text{}\left(53.0 m\text{}\right)\left(0.342\right)=18.1 m\text{}\end{array} $$ 
</div>
and

<div data-type="equation" id="eip-837">
 $$ \begin{array}{lll}{B}_{y}& =& B\sin{\theta }_{B}=\left(34.0 m\text{}\right)\left(\text{sin 63}.0º\right)\\ & =& \text{}\left(34.0 m\text{}\right)\left(0.891\right)=30.3 m\text{}\text{.}\end{array} $$ 
</div>
The **x**- and **y**-components of the resultant are thus

<div data-type="equation" id="eip-196">
 $$ {R}_{x}= A_{x}+{B}_{x}=49.8 m\text{}+15.4 m\text{}=65.2 m\text{} $$ 
</div>
and

<div data-type="equation" id="eip-325">
 $$ {R}_{y}= A_{y}+{B}_{y}=18.1 m+30.3 m=48.4 m\text{.} $$ 
</div>
Now we can find the magnitude of the resultant by using the Pythagorean theorem:

<div data-type="equation" id="eip-941">
 $$ R=\sqrt{ R_{x}^{2}+{R}_{y}^{2}}=\sqrt{\left(65.2{\right)}^{2}+\left(48.4{\right)}^{2}\text{m}} $$ 
</div>
so that
...
<div data-type="equation" id="eip-713">
 $$ \theta ={\tan}^{-1}\left({R}_{y}/{R}_{x}\right)\text{=+}{\tan}^{-1}\left(48.4/65.2\right)\text{.} $$ 
</div>
Thus,

<div data-type="equation" id="eip-755">
 $$ \theta ={\tan}^{-1}\left(0.742\right)=36.6º\text{.} $$ 
</div>
...
<div data-type="equation" id="eip-772">
 $$ {R}_{x}= A_{x}+\left(–{B}_{x}\right) $$ 
</div>
and

<div data-type="equation" id="eip-532">
 $$ {R}_{y}= A_{y}+\left(–{B}_{y}\right) $$ 
</div>
...
* {: #import-auto-id1165296238492} The steps to add vectors $$ \mathbf{A} $$
  ...
  <div data-type="equation" id="import-auto-id1165298699812">
   $$ \begin{array}{lll} A_{x}& =& A\cos{\theta} \\ {B}_{x}& =& B\cos{\theta} \end{array} $$ 
  </div>

  and

  <div data-type="equation" id="import-auto-id1165298717989">
   $$ \begin{array}{lll} A_{y}& =& A\sin{\theta} \\ {B}_{y}& =& B\sin{\theta} \text{.}\end{array} $$ 
  </div>
...
) (b) Repeat again, but now you first walk $$ 25}.0 \text{m} $$
north and then $$ 18}.0 \text{m} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Suppose a pilot flies  $$ 40.\text{0 km} $$ 

in a direction $$ \text{60º} $$
north of east and then flies $$ 30.\text{0 km} $$
