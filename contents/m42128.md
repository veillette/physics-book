...
Repeat [[link]](#eip-287) using analytical techniques, but reverse the order of the two legs of the walk and show that you get the same final result. (This problem shows that adding them in reverse order gives the same result—that is,  $$ \mathbf{\text{B + A = A + B}} $$ .) Discuss how taking another path to reach the same point might help to
