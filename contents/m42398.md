...
<div data-type="equation" id="eip-202">
 $$F=\text{IlB} \text{sin} \theta =\left(\text{20}\text{.0 A}\right)\left(0.0500 \text{m}\right)\left(1.50 \text{T}\right)\left(1\right)\text{.} $$
</div>
The units for tesla are  $$1 \text{T}=\frac{N}{A\cdot m} $$
; thus,
