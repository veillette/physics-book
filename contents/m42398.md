...
We can derive an expression for the magnetic force on a current by taking a sum of the magnetic forces on individual charges. (The forces add because they are in the same direction.) The force on an individual charge moving at the drift velocity  $${v}_{d} $$
 is given by  $$F={\text{qv}}_{d}B \sin \theta  $$
. Taking  $$B $$
 to be uniform over a length of wire  $$l $$
 and zero elsewhere, the total magnetic force on the wire is then  $$F=\left({\text{qv}}_{d}B \sin \theta \right)\left(N\right) $$
, where  $$N $$
...
, where  $$A $$
 is the cross-sectional area of the wire, then the force on the wire is  $$F=\left({\text{qv}}_{d}B \sin \theta \right)\left(\text{nAl}\right) $$
. Gathering terms,

<div data-type="equation" id="eip-987">
 $$F=\left({\text{nqAv}}_{d}\right)\text{lB} \sin \theta . $$
</div>
...
, we find that the magnetic force per unit length of wire in a uniform field is  $$\frac{F}{l}=\text{IB} \sin \theta  $$
...
The force can be found with the given information by using  $$F=\text{IlB} \sin \theta  $$
...
Entering the given values into  $$F=\text{IlB} \sin \theta  $$
 yields

<div data-type="equation" id="eip-202">
 $$F=\text{IlB} \sin \theta =\left(\text{20}\text{.0 A}\right)\left(0.0500 \text{m}\right)\left(1.50 \text{T}\right)\left(1\right)\text{.} $$
</div>
