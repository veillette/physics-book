...
[\[link\]](#import-auto-id1814176) lists representative values of the coefficient of linear expansion, which may have units of  $$1/\text{ºC} $$
 or 1/K. Because the size of a kelvin and a degree Celsius are the same, both * $$\alpha  $$
* and  $$\text{Δ}T $$
 can be expressed in units of kelvins or degrees Celsius. The equation  $$\text{Δ}L=\alpha L\text{Δ}T $$
...
<table id="import-auto-id1814176" summary="Three-column table gives the thermal expansion coefficients of various solids, liquids, and gases at twenty degrees Celsius. The left column gives the names of the materials; the middle column gives the coefficient of linear expansion of various solids; and the right column gives the coefficient of volume expansion of solids, liquids, and gases."><caption><span data-type="title">Thermal Expansion Coefficients at  $$20\text{ºC} $$<strong><sup data-type="footnote-number" id="footnote-ref1"><a data-type="footnote-link" href="#footnote1">1</a></sup></strong></span></caption><thead>  
          <tr>
            <th>Material</th>
...
Use the equation for linear thermal expansion  $$\text{Δ}L=\alpha L\text{Δ}T $$
...
<div data-type="equation" id="eip-520">
 $$\text{Δ}L=\alpha L\text{Δ}T=\left(\frac{12 \times 10^{-6} }{\text{ºC}}\right)\left(1275 \text{m}\right)\left(55\text{ºC}\right)=0.84 \text{m.} $$
</div>
**Discussion**
...
In general, objects will expand with increasing temperature. Water is the most important exception to this rule. Water expands with increasing temperature (its density *decreases*) when it is at temperatures greater than  $$4\text{ºC}\left(40\text{º}\text{F}\right) $$
. However, it expands with *decreasing* temperature when it is between  $$+4\text{ºC} $$
 and  $$0\text{ºC} $$
 $$\left(40\text{º}\text{F} $$
 to  $$32\text{º}\text{F}\right) $$
. Water is densest at  $$+4\text{ºC} $$
. (See [\[link\]](#import-auto-id2298254).) Perhaps the most striking effect of this phenomenon is the freezing of water in a pond. When water near the surface cools down to  $$4\text{ºC} $$
 it is denser than the remaining water and thus will sink to the bottom. This “turnover” results in a layer of warmer water near the surface, which is then cooled. Eventually the pond has a uniform temperature of  $$4\text{ºC} $$
. If the temperature in the surface layer drops below  $$4\text{ºC} $$
, the water is less dense than the water below, and thus stays near the top. As a result, the pond surface can completely freeze over. The ice on top of liquid water provides an insulating layer from winter’s harsh exterior air temperatures. Fish and other aquatic life can survive in  $$4\text{ºC} $$
...
<div data-type="equation" id="eip-400">
 $$\begin{array}{lll}{V}_{\text{spill}}& =& \left({\beta }_{\text{gas}}-{\beta }_{\text{s}}\right)V\text{Δ}T\\ & =& \left[\left(950-35\right) \times 10^{-6} /\text{ºC}\right]\left( 60.0 \text{L}\right)\left( 20.0\text{ºC}\right)\\ & =&  1.10 \text{L}.\end{array} $$
</div>
**Discussion**
...
<div data-type="equation" id="import-auto-id1319680">
 $$\begin{array}{lll}V& =& {V}_{0}+\text{Δ}V={V}_{0}\left(1+\beta \text{Δ}T\right)\\ & =& \left(60.00 \text{L}\right)\left[1+\left(950 \times 10^{-6} /\text{ºC}\right)\left( 35.0\text{ºC}- 15.0\text{ºC}\right)\right]\\ & =&  61.1 \text{L}\end{array} $$
</div>
</div>
</div>
...
<div data-type="solution" markdown="1">
We know how the length changes with temperature:  $$\text{Δ}L=\alpha L_{0}\text{Δ}T $$
...
<div data-type="equation" id="import-auto-id1452336">
 $$V={\left({L}_{0}+\alpha L_{0}\text{Δ}T\right)}^{3}={L}_{0}^{3}{\left(1+\alpha \text{Δ}T\right)}^{3}\text{.} $$
</div>
...
<div data-type="equation" id="import-auto-id1965368" data-element-type="problems-exercises">
 $$\text{Δ}V=\beta V_{0}\text{Δ}T\approx {3\alpha V}_{0}\text{Δ}T, \text{or} \beta \approx 3\alpha . $$
</div>
</div>
</div>
