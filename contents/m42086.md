...
the centripetal acceleration— $a=a_{c}$. Thus, the magnitude of centripetal
...
{c}=\frac{ v^{2}}{r};a_{c}={\mathrm{r\omega
...
We know that$$ F_{\text{c}}=\frac{ {\mathrm{mv}}^{2}}{r} $$ .
Thus, <div data-type="equation" id="eip-211">
$$ F_{\text{c}}=\frac{ {\mathrm{mv}}^{2}}{r}=\frac{\left(\text{900
kg}\right)\left(\text{25.0 m/s}{\right)
}^{2}}{\left(\text{500 m}\right)}=\text{1125 N.} $$
</div>
**Strategy for (b)**
...
$$ F_{\text{c}}=f={\mu }_{\text{s}}N={\mu }_{\text{s}}\text{mg}\text{.} $$ 
</div>
Now we have a relationship between centripetal force and the coefficient of friction. Using the first expression for $$ F_{\text{c}} $$ 
 from the equation
...
\left(9\text{.}{80 \text{m/s}}^{2}\right)}=0.13.
...
magnitude $$ {\text{mv}}^{2}\text{/r} $$. Because this is the crucial force and it is horizontal, we use a coordinate
...
<div data-type="equation" id="eip-584">
 $$ N\sin{\theta} =\frac{ {\mathrm{mv}}^{2}}{r}\text{.} $$ 
</div>
...
N\cos{\theta} $$, and the only other vertical force is the car’s weight. These must be equal in
magnitude; thus,

<div data-type="equation" id="eip-125">
 $$ N\cos{\theta} =\text{mg}\text{.} $$ 
</div>
...
<div data-type="equation" id="eip-285">
 $$ \begin{array}{rrr}\text{mg}\tan\left(\theta \right)& =& \frac{ {\mathrm{mv}}^{2}}{r}\\ \tan{\theta} & =& \frac{ v^{2}}{\text{rg.}}\end{array} $$ 
</div>
...
<div data-type="equation" id="eip-6">
 $$ v=\left(rg \tan{\theta} {\right)}^{1/2}\text{.} $$ 
</div>
Noting that tan 65.0º = 2.14, we obtain

<div data-type="equation" id="eip-571">
 $$ \begin{array}{lll}v& =& {\left[\left(\text{100 m}\right)\left(9.80 m{\text{/s}}^{2}\right)\left(2.14\right)\right]}^{1/2}\\ & =& \text{45.8 m/s.}\end{array} $$ 
</div>
**Discussion**
...
(a) At what angular velocity is the centripetal acceleration $$ 10 g $$
...
At what angle $$ \theta $$ below the horizontal will the cage hang when the centripetal acceleration is $$ 10 g $$
...
proportion to the ideal speed {:# import-auto-id3149885}
