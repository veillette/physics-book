...
net $$ \text{F}=ma $. For uniform circular motion, the acceleration is the
centripetal acceleration— $a=a_{c}$. Thus, the magnitude of centripetal force
${\text{F}}_{\text{c}} $$ is
...
$$ F_{\text{c}}=\frac{ mv^{2}}{r}=\frac{\left( 900 \text{kg}\right)\left(25.0
\text{m/s}{\right)
...
\text{m}\right)\left(9.80 \text{m/s}^{2}\right)}=0.13. $$ </div> (Because
...
expression for $$ \theta $$, as desired. Solving the second equation for $$ N=mg
/\left(
\cos{\theta} \right)
