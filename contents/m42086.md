...
By using the expressions for centripetal acceleration $$ a_{c}$ from $a_{c}=\frac{ v^{2}}{r};a_{c}=r  \omega^{2}$, we get two expressions for the centripetal force ${\text{F}}_{\text{c}}
...
friction (at which the tires roll but do not slip) is $$ \mu_{\text{s}}N $$, where $$ \mu_{\text{s}} $$
is the static coefficient of friction and N is the normal force. The normal
force equals the car’s weight on level ground, so that $$ N=mg $$. Thus the centripetal force in this situation is
<div data-type="equation" id="eip-109">
$$ F_{\text{c}}=f=\mu_{\text{s}}N=\mu_{\text{s}}mg \text{.} $$ 
</div>
...
<div data-type="equation" id="eip-18">
 $$ m\frac{ v^{2}}{r}=\mu_{\text{s}}mg \text{.} $$  </div>
We solve this for  $$ \mu_{\text{s}}$ , noting that mass cancels, and obtain <div data-type="equation" id="eip-99"> $\mu_{\text{s}}=\frac{ v^{2}}{\text{rg}}\text{.} $$  </div>
**Solution for (b)**

Substituting the knowns, <div data-type="equation" id="eip-971">  $$ \mu_{\text{s}}=\frac{\left(25.0 \text{m/s}{\right)}^{2}}{\left(500 \text{m}\right)
...
\begin{array}{c}{F}_{\text{c}}=m\frac{ v^{2}}{r}\\ {F}_{\text{c}}=\text{mr}{\omega }^{2}\end{array}\right\},$ because $m,$ $v,$ and $r$
...
force, being able to assume a value less than but no more than $\mu_{\text{s}}N $$ . A higher coefficient would also allow the car to negotiate the
...
<div data-type="equation" id="eip-571">
 $$ \begin{array}{lll}v& =& {\left[\left(100 \text{m}\right)\left(9.80 \text{m/s}^{2}\right)\left(2.14\right)\right]}^{1/2}\\ & =& \text{45.8 m/s.}\end{array} $$ 
</div>
**Discussion**
