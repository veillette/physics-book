...
{c}=\frac{ v^{2}}{r};{a}_{c}={\mathrm{r\omega
...
<div data-type="equation" id="eip-562">
 $$ {F}_{c}=m\frac{ v^{2}}{r};{F}_{c}=\text{mr}{\omega }^{2}. $$ 
</div>
...
<div data-type="equation" id="eip-6">
 $$ v=\left(\text{rg}\tan{\theta} {\right)}^{1/2}\text{.} $$ 
</div>
