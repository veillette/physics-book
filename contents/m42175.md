...
; that of the weights of the forearm is \\( {w}_{\text{a}} \\)

, and its load is \\( {w}_{\text{b}} \\)

. Two of these are unknown ( \\( {F}_{\text{B}} \\)
...
. But if we use the second condition and choose the pivot to be at the elbow,
...
, approximately equal to the weight supported.) Forces in muscles and joints are
...
force constant \\( k=\text{600}\phantom{\rule{0.25em}{0ex}}\text{N/m} \\)

. (a) How much is the rope stretched (past equilibrium) to provide the same
force \\( {F}_{\text{B}} \\)
