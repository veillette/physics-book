...
<div data-type="equation" id="eip-769">
 $$\frac{F}{l}=\frac{\left(4\pi  \times 10^{-7}  T\cdot \text{m/A}\right){\left(1 A\right)}^{2}}{\left(2\pi \right)\left(1 m\right)}=2 \times 10^{-7}  \text{N/m.} $$
</div>

Since  $${\mu }_{0} $$
 is exactly  $$4\pi  \times 10^{-7}  T\cdot \text{m/A} $$
 by definition, and because  $$1 \text{T}=1 \text{N/}\left(A\cdot m\right) $$
