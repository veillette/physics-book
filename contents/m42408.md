...
Now,  $$\Delta \left(\cos \theta \right)= -1.0 $$ ,
 ...
 goes from  $$0 \text{º} $$
 to  $$90 \text{º} $$ . Thus  $$\Delta \Phi =-\text{AB} $$, and 
...
104.7 rad/s is exactly 1000 rpm. We substitute this value for  $$\omega  $$
 and the information from the previous example into  $${\text{emf}}_{0}=\text{NAB}\omega  $$, yielding 
...
:  $$\text{emf}=\text{NAB}\omega  \sin \omega t $$ ,  where
   $$A $$ is the area of an
   $$N $$
    -turn coil rotated at a constant angular velocity
   $$\omega  $$ in a uniform magnetic field
