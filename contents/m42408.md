...
The generator coil shown in [\[link\]](#Figure1) is rotated through one-fourth of a revolution (from  $$\theta =0º $$
...
Charges in the wires of the loop experience the magnetic force, because they are moving in a magnetic field. Charges in the vertical wires experience forces parallel to the wire, causing currents. But those in the top and bottom segments feel a force perpendicular to the wire, which does not cause a current. We can thus find the induced emf by considering only the side wires. Motional emf is given to be ** $$\text{emf}=B\ell v $$ ,
 ...
 Here the velocity is at an angle  $$\theta  $$  with  $$B $$ ,
 so that its component perpendicular to  $$B $$
 is  $$v \sin \theta  $$
 (see [\[link\]](#Figure2)). Thus in this case the emf induced on each side is ** $$\text{emf}=B\ell v \sin \theta  $$ ,
 and they are in the same direction. The total emf around the loop is then

<div class="equation" id="eip-258">
...
This expression is valid, but it does not give emf as a function of time. To find the time dependence of emf, we assume the coil rotates at a constant angular velocity  $$\omega  $$ .
 The angle  $$\theta  $$
 is related to angular velocity by  $$\theta =\omega t $$ , so that
...
Now, linear velocity  $$v $$
 ...
 by  $$v=r\omega  $$ .
 Here  $$r=w/2 $$ ,
 so that  $$v=\left(w/2\right)\omega  $$ , and
...
Noting that the area of the loop is  $$A=\ell w $$ ,
 and allowing for  $$N $$  loops, we find that
...
Once  $$\omega  $$ ,
 the angular velocity, is determined,  $${\text{emf}}_{0}=NAB\omega  $$
...
In real life, electric generators look a lot different than the figures in this section, but the principles are the same. The source of mechanical energy that turns the coil can be falling water (hydropower), steam produced by the burning of fossil fuels, or the kinetic energy of wind. [\[link\]](#Figure5) shows a cutaway view of a steam turbine; steam moves over the blades connected to the shaft, which rotates the coil within the generator.

 ![Photograph of a steam turbine connected to a generator.](../resources/Figure_24_05_05.jpg "Steam turbine/generator. The steam produced by burning coal impacts the turbine blades, turning the shaft which is connected to the generator. (credit: Nabonaco, Wikimedia Commons)"){: #Figure5}
...
*  An electric generator rotates a coil in a magnetic field, inducing an emf**given as a function of time by
  <div class="equation" id="eip-841">
   $$\text{emf}=NAB\omega  \sin \omega t\text{,} $$
  </div>
  
  where    $$A $$ is the area of an    $$N $$
    -turn coil rotated at a constant angular velocity
   $$\omega  $$ in a uniform magnetic field    $$B $$ . 

*  The peak emf
   $${\text{emf}}_{0} $$ of a generator is
  <div class="equation" id="eip-714">
...
Using RHR-1, show that the emfs in the sides of the generator loop in [[link]](#Figure4) are in the same sense and thus add.
...
:  $$\text{emf}=NAB\omega  \sin \omega t $$ ,  where
   $$A $$ is the area of an    $$N $$
    -turn coil rotated at a constant angular velocity
   $$\omega  $$ in a uniform magnetic field
   $$B $$ ,  over a period of time    $$t $$
