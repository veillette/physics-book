...
v_{1}= v_{2}=v $$. Using the properties of two similar triangles, we obtain
...
Finally, noting that $$ \Delta v/\Delta t=a_{\text{c}} $$
...
radius, so that $$ a_{\text{c}} $$
is greater for tighter turns, as you have probably noticed.

It is also useful to express $$ a_{\text{c}} $$
in terms of angular velocity. Substituting $$ v=r \omega  $$
into the above expression, we find $$ a_{\text{c}}={\left(r \omega }\right)}^{2}/r= r \omega^{2} $$. We can express the magnitude of centripetal acceleration using either of two
equations:
...
Recall that the direction of $$ a_{\text{c}} $$
...
Because $$ v $$
and $$ r $$
are given, the first expression in $$ a_{\text{c}}=\frac{
{v}^{2}}{r}\mathrm{; }a_{\text{c}}=r \omega^{2} $$
...
v=25.0\text{m/s} $$
and $$ r=500 \text{m} $$
into the first expression for $$ a_{\text{c}} $$
gives
...
=0.128 $$. Thus, $$ a_{\text{c}}=0.128 \text{g} $$
...
Calculate the centripetal acceleration of a point 7.50 cm from the axis of an **ultracentrifuge**{: data-type="term" #import-auto-id2449410} spinning at  $$ {\text{7.5 × 10}}^{4}\text{rev/min.} $$ 
...
is given, we can use the second expression in the equation $$ a_{\text{c}}=\frac{ v^{2}}{r};a_{\text{c}}=r \omega^{2} $$
to calculate the centripetal acceleration.

**Solution**

To convert $$ 7.50\times 10^{4}\text{rev}/\text{min}
$$
...
Now the centripetal acceleration is given by the second expression in  $$ a_{\text{c}}=\frac{ v^{2}}{r}\mathrm{; }a_{\text{c}}=r \omega^{2} $$ 
...
Note that the unitless radians are discarded in order to get the correct units for centripetal acceleration. Taking the ratio of  $$ a_{\text{c}} $$ 
...
* {: #import-auto-id1963039} Centripetal acceleration $$ a_{\text{c}} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Taking the age of Earth to be about  $$ 4\times 10^{9} $$ 

years and assuming its orbital radius of $$ 1.5 \times 10^{11} $$
...
<div data-type="solution" markdown="1">
a)  $$ 3.47\times 10^{4}\text{m}/{\text{s}}^{2} $$ 
...
(d)  $$ 1.76\times 10^{3}\text{N
or 3}.00w $$, that is, the normal force (upward) is three times her weight.
