...
When are resistors in **series**? Resistors are in series whenever the flow of charge, called the **current**, must flow through devices sequentially. For example, if current flows through a person holding a screwdriver and into the Earth, then  $${R}_{1} $$
 in [\[link\]](#import-auto-id2989840)(a) could be the resistance of the screwdriver’s shaft,  $${R}_{2} $$
 the resistance of its handle,  $${R}_{3} $$
 the person’s body resistance, and  $${R}_{4} $$
...
To verify that resistances in series do indeed add, let us consider the loss of electrical power, called a **voltage drop**, in each resistor in [\[link\]](#import-auto-id3043995).

According to **Ohm’s law**, the voltage drop,  $$V $$
, across a resistor when a current flows through it is calculated using the equation  $$V=\text{IR} $$
, where  $$I $$
 equals the current in amps (A) and  $$R $$
 is the resistance in ohms  $$\left(\Omega \right) $$
. Another way to think of this is that  $$V $$
 is the voltage necessary to make a current  $$I $$
 flow through a resistance  $$R $$
.

So the voltage drop across  $${R}_{1} $$
 is  $${V}_{1}={\mathit{IR}}_{1} $$
, that across  $${R}_{2} $$
 is  $${V}_{2}={\mathit{IR}}_{2} $$
, and that across  $${R}_{3} $$
 is  $${V}_{3}={\mathit{IR}}_{3} $$
...
This equation is based on the conservation of energy and conservation of charge. Electrical potential energy can be described by the equation  $$\text{PE}=\text{qV} $$
, where  $$q $$
 is the electric charge and  $$V $$
 is the voltage. Thus the energy supplied by the source is  $$\text{qV} $$
...
These energies must be equal, because there is no other source and no other destination for energy in the circuit. Thus,  $$\mathit{qV}={\mathit{qV}}_{1}+{\mathit{qV}}_{2}+{\mathit{qV}}_{3} $$
. The charge  $$q $$
 cancels, yielding  $$V={V}_{1}+{V}_{2}+{V}_{3} $$
...
Note that for the equivalent single series resistance  $${R}_{\text{s}} $$
, we have
...
This logic is valid in general for any number of resistors in series; thus, the total resistance  $${R}_{\text{s}} $$
...
Suppose the voltage output of the battery in [\[link\]](#import-auto-id3043995) is  $$\text{12}\text{.}0 \text{V} $$
, and the resistances are  $${R}_{1}=1\text{.}\text{00} \Omega  $$
,  $${R}_{2}=6\text{.}\text{00} \Omega  $$
, and  $${R}_{3}=\text{13}\text{.}0 \Omega  $$
...
The current is found using Ohm’s law,  $$V=\text{IR} $$
...
<div data-type="equation" id="eip-343">
 $$I=\frac{V}{ {R}_{\text{s}}}=\frac{\text{12}\text{.}0 \text{V}}{\text{20}\text{.}\text{0} \Omega }=0\text{.}\text{600} \text{A}. $$
</div>
**Strategy and Solution for (c)**

The voltage—or  $$\text{IR} $$
...
<div data-type="equation" id="eip-106">
 $${V}_{1}={\mathit{IR}}_{1}=\left(0\text{.}\text{600} \text{A}\right)\left(1\text{.}0 \Omega \right)=0\text{.}\text{600} \text{V}. $$
</div>
Similarly,

<div data-type="equation" id="eip-787">
 $${V}_{2}={\mathit{IR}}_{2}=\left(0\text{.}\text{600} \text{A}\right)\left(6\text{.}0 \Omega \right)=3\text{.}\text{60} \text{V} $$
</div>
and

<div data-type="equation" id="eip-917">
 $${V}_{3}={\mathit{IR}}_{3}=\left(0\text{.}\text{600} \text{A}\right)\left(\text{13}\text{.}0 \Omega \right)=7\text{.}\text{80} \text{V}. $$
</div>
**Discussion for (c)**

The three  $$\text{IR} $$
 drops add to  $$\text{12}\text{.}0 \text{V} $$
, as predicted:

<div data-type="equation" id="eip-359">
 $${V}_{1}+{V}_{2}+{V}_{3}=\left(0\text{.}\text{600}+3\text{.}\text{60}+7\text{.}\text{80}\right) \text{V}=\text{12}\text{.}0 \text{V}. $$
</div>
**Strategy and Solution for (d)**

The easiest way to calculate power in watts (W) dissipated by a resistor in a DC circuit is to use **Joule’s law**,  $$P=\text{IV} $$
, where  $$P $$
 is electric power. In this case, each resistor has the same full current flowing through it. By substituting Ohm’s law  $$V=\text{IR} $$
 into Joule’s law, we get the power dissipated by the first resistor as

<div data-type="equation" id="eip-839">
 $${P}_{1}={I}^{2}{R}_{1}=\left(0\text{.}\text{600} \text{A}{\right)}^{2}\left(1\text{.}\text{00} \Omega \right)=0\text{.}\text{360} \text{W}. $$
</div>
Similarly,

<div data-type="equation" id="eip-192">
 $${P}_{2}={I}^{2}{R}_{2}=\left(0\text{.}\text{600} \text{A}{\right)}^{2}\left(6\text{.}\text{00} \Omega \right)=2\text{.}\text{16} \text{W} $$
</div>
and

<div data-type="equation" id="eip-997">
 $${P}_{3}={I}^{2}{R}_{3}=\left(0\text{.}\text{600} \text{A}{\right)}^{2}\left(\text{13}\text{.}0 \Omega \right)=4\text{.}\text{68} \text{W}. $$
</div>
**Discussion for (d)**

Power can also be calculated using either  $$P=\text{IV} $$
 or  $$P=\frac{ {V}^{2}}{R} $$
, where  $$V $$
...
The easiest way to calculate power output of the source is to use  $$P=\text{IV} $$
, where  $$V $$
 is the source voltage. This gives

<div data-type="equation" id="eip-271">
 $$P=\left(0\text{.}\text{600} \text{A}\right)\left(\text{12}\text{.}0 \text{V}\right)=7\text{.}\text{20} \text{W}. $$
</div>
**Discussion for (e)**
...
<div data-type="equation" id="eip-698">
 $${P}_{1}+{P}_{2}+{P}_{3}=\left(0\text{.}\text{360}+2\text{.}\text{16}+4\text{.}\text{68}\right) \text{W}=7\text{.}\text{20} \text{W}. $$
</div>
...
[\[link\]](#import-auto-id1926821) shows resistors in **parallel**, wired to a voltage source. Resistors are in parallel when each resistor is connected directly to the voltage source by connecting wires having negligible resistance. Each resistor thus has the full voltage of the source applied to it.
...
, let us consider the currents that flow and how they are related to resistance. Since each resistor in the circuit has the full voltage, the currents flowing through the individual resistors are  $${I}_{1}=\frac{V}{ {R}_{1}} $$
,  $${I}_{2}=\frac{V}{ {R}_{2}} $$
, and  $${I}_{3}=\frac{V}{ {R}_{3}} $$
. Conservation of charge implies that the total current  $$I $$
...
This relationship results in a total resistance  $${R}_{p} $$
...
Let the voltage output of the battery and resistances in the parallel connection in [\[link\]](#import-auto-id1926821) be the same as the previously considered series connection:  $$V=\text{12}\text{.}0 \text{V} $$
,  $${R}_{1}=1\text{.}\text{00} \Omega  $$
,  $${R}_{2}=6\text{.}\text{00} \Omega  $$
, and  $${R}_{3}=\text{13}\text{.}0 \Omega  $$
...
<div data-type="equation" id="eip-319">
 $$\frac{1}{ {R}_{p}}=\frac{1}{ {R}_{1}}+\frac{1}{ {R}_{2}}+\frac{1}{ {R}_{3}}=\frac{1}{1\text{.}\text{00} \Omega }+\frac{1}{6\text{.}\text{00} \Omega }+\frac{1}{\text{13}\text{.}0 \Omega }. $$
</div>
Thus,
...
We must invert this to find the total resistance  $${R}_{\text{p}} $$
. This yields

<div data-type="equation" id="eip-581">
 $${R}_{\text{p}}=\frac{1}{1\text{.}\text{2436}}\Omega =0\text{.}\text{8041} \Omega . $$
</div>
The total resistance with the correct number of significant digits is  $${R}_{\text{p}}=0\text{.}\text{804} \Omega . $$
**Discussion for (a)**
...
<div data-type="equation" id="eip-477">
 $${I}_{1}=\frac{V}{ {R}_{1}}=\frac{\text{12}\text{.}0 \text{V}}{1\text{.}\text{00} \Omega }=\text{12}\text{.}0 \text{A}. $$
</div>
Similarly,

<div data-type="equation" id="eip-224">
 $${I}_{2}=\frac{V}{ {R}_{2}}=\frac{\text{12}\text{.}0 \text{V}}{6\text{.}\text{00} \Omega }=2\text{.}\text{00} \text{A} $$
</div>
and

<div data-type="equation" id="eip-639">
 $${I}_{3}=\frac{V}{ {R}_{3}}=\frac{\text{12}\text{.}0 \text{V}}{\text{13}\text{.}\text{0} \Omega }=0\text{.}\text{92} \text{A}. $$
</div>
**Discussion for (c)**
...
<div data-type="equation" id="eip-684">
 $${I}_{1}+{I}_{2}+{I}_{3}=\text{14}\text{.}\text{92} \text{A}. $$
</div>
This is consistent with conservation of charge.

**Strategy and Solution for (d)**

The power dissipated by each resistor can be found using any of the equations relating power to current, voltage, and resistance, since all three are known. Let us use  $$P=\frac{ {V}^{2}}{R} $$
, since each resistor gets full voltage. Thus,

<div data-type="equation" id="eip-610">
 $${P}_{1}=\frac{ {V}^{2}}{ {R}_{1}}=\frac{\left(\text{12}\text{.}0 \text{V}{\right)}^{2}}{1\text{.}\text{00} \Omega }=\text{144} \text{W}. $$
</div>
Similarly,

<div data-type="equation" id="eip-43">
 $${P}_{2}=\frac{ {V}^{2}}{ {R}_{2}}=\frac{\left(\text{12}\text{.}0 \text{V}{\right)}^{2}}{6\text{.}\text{00} \Omega }=\text{24}\text{.}0 \text{W} $$
</div>
and

<div data-type="equation" id="eip-326">
 $${P}_{3}=\frac{ {V}^{2}}{ {R}_{3}}=\frac{\left(\text{12}\text{.}0 \text{V}{\right)}^{2}}{\text{13}\text{.}\text{0} \Omega }=\text{11}\text{.}1 \text{W}. $$
</div>
**Discussion for (d)**
...
The total power can also be calculated in several ways. Choosing  $$P=\text{IV} $$
...
<div data-type="equation" id="eip-286">
 $${P}_{1}+{P}_{2}+{P}_{3}=\text{144} \text{W}+\text{24}\text{.}0 \text{W}+\text{11}\text{.}1 \text{W}=\text{179} \text{W}. $$
</div>
...
1.  {: #import-auto-id2600367} Parallel resistance is found from
     $$\frac{1}{ {R}_{\text{p}}}=\frac{1}{ {R}_{1}}+\frac{1}{ {R}_{2}}+\frac{1}{ {R}_{3}}+\text{.}\text{.}\text{.} $$
...
The simplest combination of series and parallel resistance, shown in [\[link\]](#import-auto-id3085827), is also the most instructive, since it is found in many applications. For example,  $${R}_{1} $$
 could be the resistance of wires from a car battery to its electrical devices, which are in parallel.  $${R}_{2} $$
 and  $${R}_{3} $$
...
[\[link\]](#import-auto-id3085827) shows the resistors from the previous two examples wired in a different way—a combination of series and parallel. We can consider  $${R}_{1} $$
 to be the resistance of wires leading to  $${R}_{2} $$
 and  $${R}_{3} $$
. (a) Find the total resistance. (b) What is the  $$\text{IR} $$
 drop in  $${R}_{1} $$
...
<div data-type="equation" id="eip-7">
 $$\frac{1}{ {R}_{\text{p}}}=\frac{1}{ {R}_{2}}+\frac{1}{ {R}_{3}}=\frac{1}{6\text{.}\text{00} \Omega }+\frac{1}{\text{13}\text{.}0 \Omega }=\frac{0\text{.}\text{2436}}{\Omega }. $$
</div>
Inverting gives

<div data-type="equation" id="eip-671">
 $${R}_{\text{p}}=\frac{1}{0\text{.}\text{2436}}\Omega =4\text{.}\text{11} \Omega . $$
</div>
So the total resistance is

<div data-type="equation" id="eip-114">
 $${R}_{\text{tot}}={R}_{1}+{R}_{\text{p}}=1\text{.}\text{00} \Omega +4\text{.}\text{11} \Omega =5\text{.}\text{11} \Omega . $$
</div>
**Discussion for (a)**

The total resistance of this combination is intermediate between the pure series and pure parallel values ( $$20.0 \Omega  $$
 and  $$0.804 \Omega  $$
...
To find the  $$\text{IR} $$
 drop in  $${R}_{1} $$
, we note that the full current  $$I $$
 flows through  $${R}_{1} $$
. Thus its  $$\text{IR} $$
 drop is
...
<div data-type="equation" id="eip-262">
 $$I=\frac{V}{ {R}_{\text{tot}}}=\frac{\text{12}\text{.}0 \text{V}}{5\text{.}\text{11} \Omega }=2\text{.}\text{35} \text{A}. $$
</div>
Entering this into the expression above, we get

<div data-type="equation" id="eip-316">
 $${V}_{1}={\text{IR}}_{1}=\left(2\text{.}\text{35} \text{A}\right)\left(1\text{.}\text{00} \Omega \right)=2\text{.}\text{35} \text{V}. $$
</div>
**Discussion for (b)**

The voltage applied to  $${R}_{2} $$
 and  $${R}_{3} $$
 is less than the total voltage by an amount  $${V}_{1} $$
. When wire resistance is large, it can significantly affect the operation of the devices represented by  $${R}_{2} $$
 and  $${R}_{3} $$
.

**Strategy and Solution for (c)**

To find the current through  $${R}_{2} $$
, we must first find the voltage applied to it. We call this voltage  $${V}_{\text{p}} $$
, because it is applied to a parallel combination of resistors. The voltage applied to both  $${R}_{2} $$
 and  $${R}_{3} $$
 is reduced by the amount  $${V}_{1} $$
, and so it is

<div data-type="equation" id="eip-984">
 $${V}_{\text{p}}=V-{V}_{1}=\text{12}\text{.}0 \text{V}-2\text{.}\text{35} \text{V}=9\text{.}\text{65} \text{V}. $$
</div>
...
<div data-type="equation" id="eip-376">
 $${I}_{2}=\frac{ {V}_{\text{p}}}{ {R}_{2}}=\frac{9\text{.}\text{65 V}}{6\text{.}\text{00} \Omega }=1\text{.}\text{61} \text{A}. $$
</div>
**Discussion for (c)**

The current is less than the 2.00 A that flowed through  $${R}_{2} $$
 when it was connected in parallel to the battery in the previous parallel circuit example.

**Strategy and Solution for (d)**

The power dissipated by  $${R}_{2} $$
 is given by

<div data-type="equation" id="eip-12">
 $${P}_{2}=\left({I}_{2}{\right)}^{2}{R}_{2}=\left(1\text{.}\text{61} \text{A}{\right)}^{2}\left(6\text{.}\text{00} \Omega \right)=\text{15}\text{.}5 \text{W}. $$
</div>
**Discussion for (d)**
...
One implication of this last example is that resistance in wires reduces the current and power delivered to a resistor. If wire resistance is relatively large, as in a worn (or a very long) extension cord, then this loss can be significant. If a large current is drawn, the  $$\text{IR} $$
...
What is happening in these high-current situations is illustrated in [\[link\]](#import-auto-id2452527). The device represented by  $${R}_{3} $$
 has a very low resistance, and so when it is switched on, a large current flows. This increased current causes a larger  $$\text{IR} $$
 drop in the wires represented by  $${R}_{1} $$
, reducing the voltage across the light bulb (which is  $${R}_{2} $$
), which then dims noticeably.

 ![A conceptual drawing showing a refrigerator with its motor and light bulbs connected to a household A C circuit through a wire with resistance of R sub one. The bulb has a resistance R sub two, and the motor has a resistance R sub three.](../resources/Figure_22_01_07.jpg "Why do lights dim when a large appliance is switched on? The answer is that the large current the appliance motor draws causes a significant \( IR \)  drop in the wires and reduces the voltage across the light."){: #import-auto-id2452527}
...
![This diagram shows a circuit with a voltage source and internal resistance small r. A resistance R and an open switch are connected in parallel to it.](../resources/Figure_22_01_09.jpg "A wiring mistake put this switch in parallel with the device represented by \( R \) . (Note that in this diagram, the script E represents the voltage (or electromotive force) of the battery.)"){: #import-auto-id1617216}
...
<div data-type="problem" markdown="1">
(a) What is the resistance of ten  $$\text{275-Ω} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) What is the resistance of a  $$\text{1.00}×{10}^{2}-\Omega  $$
, a  $$2\text{.}\text{50-kΩ} $$
, and a  $$4\text{.}\text{00-k}\Omega  $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
What are the largest and smallest resistances you can obtain by connecting a  $$\text{36}\text{.}0-\Omega  $$
, a  $$\text{50}\text{.}0-\Omega  $$
, and a  $$\text{700-Ω} $$
 resistor together?
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) Given a 48.0-V battery and  $$\text{24}\text{.}0-\Omega  $$
 and  $$\text{96}\text{.}0-\Omega  $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Referring to the example combining series and parallel circuits and [[link]](#import-auto-id3085827), calculate  $${I}_{3} $$
 in the following two different ways: (a) from the known values of  $$I $$
 and  $${I}_{2} $$
; (b) using Ohm’s law for  $${R}_{3} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Referring to [[link]](#import-auto-id3085827): (a) Calculate  $${P}_{3} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Refer to [[link]](#import-auto-id2452527) and the discussion of lights dimming when a heavy appliance comes on. (a) Given the voltage source is 120 V, the wire resistance is  $$0\text{.}\text{400} \Omega  $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
A 240-kV power transmission line carrying  $$5.00×{10}^{2} \text{A} $$
 is hung from grounded metal towers by ceramic insulators, each having a  $$1\text{.}\text{00}×{\text{10}}^{9}-\Omega  $$
...
Show that if two resistors  $${R}_{1} $$
 and  $${R}_{2} $$
 are combined and one is much greater than the other ( $${R}_{1}\text{>>}{R}_{2} $$
): (a) Their series resistance is very nearly equal to the greater resistance  $${R}_{1} $$
. (b) Their parallel resistance is very nearly equal to smaller resistance  $${R}_{2} $$
.

</div>
<div data-type="solution" markdown="1">
(a)  $$\begin{array}{}{R}_{\text{s}}={R}_{1}+{R}_{2}\\ ⇒{R}_{\text{s}} \approx  {R}_{1}\left({R}_{1}\text{>>}{R}_{2}\right)\end{array} $$
(b)  $$\frac{1}{ {R}_{p}}=\frac{1}{ {R}_{1}}+\frac{1}{ {R}_{2}}=\frac{ {R}_{1}+{R}_{2}}{ {R}_{1}{R}_{2}} $$
,

so that

 $$\begin{array}{}{R}_{p}=\frac{ {R}_{1}{R}_{2}}{ {R}_{1}+{R}_{2}}\approx \frac{ {R}_{1}{R}_{2}}{ {R}_{1}}={R}_{2}\left({R}_{1}\text{>>}{R}_{2}\right)\text{.}\end{array} $$
</div>
</div>
...
Two resistors, one having a resistance of  $$1\text{45} \Omega  $$
, are connected in parallel to produce a total resistance of  $$150 \Omega  $$
...
Two resistors, one having a resistance of  $$9\text{00 kΩ} $$
, are connected in series to produce a total resistance of  $$0\text{.}\text{500 MΩ} $$
. (a) What is the value of the second resistance? (b) What is unreasonable about this result? (c) Which assumptions are unreasonable or inconsistent?
