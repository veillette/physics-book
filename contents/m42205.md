...
* are in common use. For example, the heart of a resting adult pumps blood at a rate of 5.00 liters per minute (L/min). Note that a **liter** (L) is 1/1000 of a cubic meter or 1000 cubic centimeters ( $$10^{-3} {\text{m}}^{3} $$
<sup /> or  $$10^{3} {\text{cm}}^{3} $$
...
<div data-type="equation">
 $${n}_{2}=\frac{\left(1\right)\left(\pi \right){\left(10 \times 10^{-3}  \text{m}\right)}^{2}\left(0.27 m/s\right)}{\left(\pi \right){\left( 4.0 \times 10^{-6}  \text{m}\right)}^{2}\left( 0.33 \times 10^{-3}  \text{m/s}\right)}= 5.0 \times 10^{9}  \text{capillaries}. $$
</div>
**Discussion**
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) Estimate the time it would take to fill a private swimming pool with a capacity of 80 000 L using a garden hose delivering 60 L/min. (b) How long would it take to fill if you could divert a moderate size river, flowing at  $$5000 {\text{m}}^{3}\text{/s} $$
, into it?
