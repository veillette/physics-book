...
( $$ F=ma $$), where torque is analogous to force, angular acceleration is analogous to
...
( $$ \text{kg}\cdot \text{m}^{2} $$), as we might expect from its definition.
...
(at $$ L/2 $$)? (That would be $$ ML^{2}\text{/4} $$.)
