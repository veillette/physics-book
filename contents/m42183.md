...
<div data-type="equation" id="eip-177">
 $$ I^\prime =\left(0.0500\text{kg}+0.667 \text{kg}\right){\left(1.20\text{m}
\right)
}^{2}=1.032\text{kg}\cdot \text{m}^{2}. $$ 
</div>
The value of  $$ I^{\prime}  $$ is now entered into the expression for $$ \omega^\prime $$, which yields

<div data-type="equation" id="eip-74">
 $$ \begin{array}{lll}\omega^\prime & =& \frac{ mvr }{I^{\prime} }=\frac
{\left(0.0500 \text{kg}\right)\left(30.0 m/s\right)\left(1.20 \text{m}\right)}{1.032 \text{kg}\cdot \text{m}^{2}}\\ & =& 1.744 \text{rad/s}\approx 1.74 \text{rad/s}.\end{array} $$ 
</div>
**Strategy for (b)**
...
<div data-type="equation" id="eip-100">
 $$ \begin{array}{lll}
\text{KE}^\prime & =& 
\frac{1}{2}I^{\prime} {\omega^\prime}^{2}
=\left(0.5\right)\left(1.032\text{kg}
\cdot \text{m}^{2}\right){\left(1.744\frac{\text{rad}}{\text{s}}\right)}^{2}\\
& =& 1.57 \text{J.}\end{array} $$ 
</div>
**Strategy for (c)**
...
* {: #import-auto-id2603851} Angular momentum is conserved if the net torque is
  zero $$ L=\text{constant}\left( \text{net}\tau =0 \right) $$ or $$ L=L^
  {\prime} \left( \text{net}\tau = 0 \right) $$ . This equation is known as 
  the law of
