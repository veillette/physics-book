...
<div data-type="equation" id="eip-448">
 $$\begin{array}{lll}C& =& \frac{2{E}_{\text{cap}}}{ {V}^{2}}=\frac{2\left( 4.00 \times 10^{2}  \text{J}\right)}{\left( 1.00 \times 10^{4}  \text{V}{\right)}^{2}}=8\text{.}\text{00}×{\text{10}}^{–6} \text{F}\\ & =& \text{8}\text{.}\text{00 µF}\text{.}\end{array} $$
</div>
**Discussion**
