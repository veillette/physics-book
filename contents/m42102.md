...
, final position $$ y=\text{−30}.0 \text{m} $$, and $$ a=-g=-9\text{.}{80 \text{m/s}}^{2} $$. We can then use the equation $$ y=y_{0}+v_{0}t+\frac{1}{2}{\text{at}}^{2}
$$
to solve for $$ t $$. Inserting $$ a=-g $$, we obtain

<div data-type="equation" id="import-auto-id2358783">
 $$ \begin{array}{lll}y& =& 0+0-\frac{1}{2}{\text{gt}}^{2}\\ {t}^{2}& =& \frac{2y}{-g}\\ t& =& ±\sqrt{\frac{2y}{-g}}=±\sqrt{\frac{2\left(-30.0 \text{m}\right)}{-9.80 m{\text{/s}}^{2}}}=±\sqrt{6.12{s}^{2}}=2.47 \text{s}\approx 2.5 \text{s}\end{array} $$ 
</div>
