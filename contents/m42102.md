...
so important that its magnitude is given its own symbol, $$ g $$. It is constant at any given location on Earth and has the average value

<div data-type="equation" id="eip-636">
 $$ g=9.80 \text{m/s}^{2}\text{.} $$ 
</div>

Although $$ g $$
varies from $$ 9.78 \text{m/s}^{2} $$
to $$  $$
$$ 9.83 \text{m/s}^{2} $$, depending on latitude, altitude, underlying geological formations, and local
topography, the average value of $$ 9.{80 \text{m/s}}^{2} $$
...
a=-g=-9.80 \text{m/s}^{2} $$, and if we define the downward direction as positive, then $$
a=g=9.80 \text{m/s}^{2} $$.
...
constant acceleration of magnitude $$ g $$. We will also represent vertical displacement with the symbol $$ y $$
and use $$ x $$
...
<strong>Solution for Position </strong>  $$ {y}_{1} $$ 1\. Identify the knowns. We know that $$ {y}_{0}=0 $$
; $$ {v}_{0}=13.0 \text{m/s} $$
; $$ a=-g=-9.{80 \text{m/s}}^{2} $$
; and $$ t=1.00 \text{s} $$.
...
(or $$ {y}_{1} $$, here),** which is the value we want to find.

3\. Plug in the known values and solve for $$ {y}_{1} $$.

<div data-type="equation" id="import-auto-id2302522">
 $$ y{}_{1}\text{}=0+\left(13.0 \text{m/s}\right)\left(1.00 \text{s}\right)+\frac{1}{2}\left(-9.80{\text{m/s}}^{2}\right){\left(1.00 \text{s}\right)}^{2}=8.10\text{m} $$ 
</div>
**Discussion**

The rock is 8.10 m above its starting point at $$ t=1.00 $$
s, since $$ {y}_{1}>{y}_{0} $$. It could be *moving* up or down; the only way to tell is to calculate $$ {v}_
{1} $$
and find out if it is positive or negative.

<strong>Solution for Velocity </strong>  $$ {v}_{1} $$ 1\. Identify the knowns. We know that $$ {y}_{0}=0 $$
; $$ {v}_{0}=13.0 \text{m/s} $$
; $$ a=-g=-9\text{.}{80 \text{m/s}}^{2} $$
; and $$ t=1.00 \text{s} $$. We also know from the solution above that $$ {y}_{1}=8.10 \text{m} $$.

2\. Identify the best equation to use. The most straightforward is $$ v={v}_
{0}-\text{gt} $$
(from $$ v={v}_{0}+\text{at} $$, where $$ a=\text{gravitational acceleration}=-g $$
).

3\. Plug in the knowns and solve.

<div data-type="equation" id="import-auto-id1688803">
 $$ {v}_{1}={v}_{0}-\text{gt}=13.0 \text{m/s}-\left(9\text{.}{80 \text{m/s}}^{2}\right)\left(1.00 \text{s}\right)=3.20 \text{m/s} $$ 
</div>
**Discussion**

The positive value for $$ {v}_{1} $$
means that the rock is still heading upward at $$
t=1.00\text{s} $$. However, it has slowed from its original 13.0** m/s, as expected.
...
m/s}}^{2} $$. Its acceleration is $$ -9\text{.}{80 \text{m/s}}^{2} $$
...
finishes below the starting point at $$ {y}_{0}=0 $$. Similarly, the initial velocity is downward and therefore negative, as is the
...
; $$ {y}_{1}=-5.10 \text{m} $$
; $$ {v}_{0}=-13\text{.0 m/s} $$
; $$ a=-g=-9.80 \text{m}{\text{/s}}^{2} $$.

2\. Choose the kinematic equation that makes it easiest to solve the problem.
The equation $$ {v}^{2}={v}_
{0}^{2}+2a\left(y-{y}_{0}\right) $$
works well because the only unknown in it is $$ v $$. (We will plug $$ {y}_{1} $$
in for $$ y $$.)

3\. Enter the known values

<div data-type="equation" id="import-auto-id2025017">
 $$ {v}^{2}={\left(-13.0 \text{m/s}\right)}^{2}+2\left(-9\text{.}{80 \text{m/s}}^{2}\right)\left(-5.10 \text{m}-0 \text{m}\right)=268.{96 \text{m}}^{2}{\text{/s}}^{2}, $$ 
</div>
...
thrown up with an initial velocity of $$ 13\text{.0 m/s} $$. It rises and then falls back down. When its position is $$ y=0 $$
on its way back down, its velocity is $$ -13\text{.0 m/s} $$. That is, it has the same speed on its way down as on its way up. We would then
expect its velocity at a position of $$ y=-5.10 \text{m} $$
to be the same whether we have thrown it upwards at $$ +13\text{.0 m/s}
$$
or thrown it downwards at $$ -13\text{.0 m/s} $$. The velocity of the rock on its way down from $$ y=0 $$
...
and rearrange the equation to solve for $$ a $$. Substituting 0 for $$ {v}_{0} $$
yields
...
<div data-type="equation" id="import-auto-id2571438">
 $$ a=\frac{2\left(-1\text{.}\text{0000 m – 0}\right)}{\left(0.45173 \text{s}{\right)}^{2}}=-9\text{.}{8010 \text{m/s}}^{2}, $$ 
</div>
so, because  $$ a=-g $$ 
...
<div data-type="equation" id="import-auto-id1706716">
 $$ g=9\text{.}{8010 \text{m/s}}^{2}. $$ 
</div>
**Discussion**

The negative value for $$ a $$
indicates that the gravitational acceleration is downward, as expected. We
expect the value to be somewhere around the average value of $$ 9\text{.}{80 \text{m/s}}^{2} $$, so $$ 9\text{.}{8010 \text{m/s}}^{2} $$
makes sense. Since the data going into the calculation are relatively precise,
this value for $$ g $$
is more precise than the average value of $$ 9\text{.}{80 \text{m/s}}^{2} $$
...
, final position $$ y=\text{−30}.0 \text{m} $$, and $$ a=-g=-9\text{.}{80 \text{m/s}}^{2} $$. We can then use the equation $$ y={y}_{0}+{v}_{0}t+\frac{1}{2}{\text{at}}^{2}
$$
to solve for $$ t $$. Inserting $$ a=-g $$, we obtain

<div data-type="equation" id="import-auto-id2358783">
 $$ \begin{array}{lll}y& =& 0+0-\frac{1}{2}{\text{gt}}^{2}\\ {t}^{2}& =& \frac{2y}{-g}\\ t& =& ±\sqrt{\frac{2y}{-g}}=±\sqrt{\frac{2\left(-\text{30.0 m}\right)}{-9.80 m{\text{/s}}^{2}}}=±\sqrt{6.12{s}^{2}}=\text{2.47 s}\approx \text{2.5 s}\end{array} $$ 
</div>
...
<div data-type="solution" markdown="1">
(a)  $$ a=-9\text{.}{80 \text{m/s}}^{2} $$ 

; $$ {v}_{0}=13.0 \text{m/s} $$
; $$ {y}_{0}=0 \text{m} $$
(b)  $$ v=0\text{m/s} $$. Unknown is distance $$ y $$
to top of trajectory, where velocity is zero. Use equation $$ {v}^{2}={v}_
{0}^{2}+2a\left(y-{y}_{0}\right) $$
because it contains all known values except for $$ y $$, so we can solve for $$ y $$. Solving for $$ y $$
gives

<div data-type="equation" id="eip-id2418613">
 $$ \begin{array}{lll}{v}^{2}-{v}_{0}^{2}& =& 2a\left(y-{y}_{0}\right)\\ \frac{ v^{2}-{v}_{0}^{2}}{2a}& =& y-{y}_{0}\\ y& =& {y}_{0}+\frac{ v^{2}-{v}_{0}^{2}}{2a}=0 m+\frac{ {\left(0 \text{m/s}\right)}^{2}-{\left(\text{13.0 m/s}\right)}^{2}}{2\left(-\text{9.80 m}{\text{/s}}^{2}\right)}=\text{8.62 m}\end{array} $$ 
</div>
...
free-fall
: the state of movement that results from gravitational force only {: #import-auto-id2801708}
