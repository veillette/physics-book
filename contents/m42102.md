...
<div data-type="equation" id="import-auto-id2358783">
 $$ \begin{array}{lll}y& =& 0+0-\frac{1}{2}g t^{2} \\ {t}^{2}& =& \frac{2y}{-g}\\ t& =&  \pm  \sqrt{\frac{2y}{-g}}= \pm  \sqrt{\frac{2\left(-30.0 \text{m}\right)}{-9.80 \text{m/s}^{2}}^{2}}}= \pm  \sqrt{6.12{s}^{2}}=2.47 \text{s}\approx 2.5 \text{s}
\end{array} $$ 
</div>
