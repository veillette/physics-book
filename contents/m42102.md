...
2\. Identify the best equation to use. We will use $$ y=y_{0}+v_
{0}t+\frac{1}{2} at^{2} $$
...
s, since $$ y_{1}>y_{0} $$. It could be *moving* up or down; the only way to tell is to calculate $$ v_
{1} $$
...
<div data-type="equation" id="import-auto-id2358783">
 $$ \begin{array}{lll}y& =& 0+0-\frac{1}{2}g t^{2} \\ {t}^{2}& =& \frac{2y}{-g}\\ t& =& ±\sqrt{\frac{2y}{-g}}=±\sqrt{\frac{2\left(-30.0 \text{m}\right)}{-9.80 m{\text{/s}}^{2}}}=±\sqrt{6.12{s}^{2}}=2.47 \text{s}\approx 2.5 \text{s}\end{array} $$ 
</div>
...
to top of trajectory, where velocity is zero. Use equation $$ {v}^{2}=v_
{0}^{2}+2a\left(y-y_{0}\right) $$
