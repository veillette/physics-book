...
to $$  $$
$$ 9\text{.}{\text{83 m/s}}^{2} $$, depending on latitude, altitude, underlying geological formations, and local
...
a=-g=-9\text{.}{\text{80 m/s}}^{2} $$, and if we define the downward direction as positive, then $$
...
<div data-type="equation" id="import-auto-id2302522">
 $$ y{}_{1}\text{}=0+\left(\text{13}\text{.}\text{0 m/s}\right)\left(1\text{.}\text{00 s}\right)+\frac{1}{2}\left(-9\text{.}\text{80}{\text{m/s}}^{2}\right){\left(1\text{.}\text{00 s}\right)}^{2}=8\text{.}\text{10}\text{m} $$ 
</div>
**Discussion**
...
; $$ {v}_{0}=\text{13}\text{.}\text{0 m/s} $$
; $$ a=-g=-9\text{.}{\text{80 m/s}}^{2} $$
; and $$ t=1\text{.}\text{00 s} $$. We also know from the solution above that $$ {y}_{1}=8\text{.}\text{10 m} $$.
...
t=1\text{.}\text{00}\text{s} $$. However, it has slowed from its original 13.0** m/s, as expected.

**Solution for Remaining Times**

The procedures for calculating the position and velocity at $$
t=2\text{.}\text{00}\text{s} $$
and $$ 3\text{.}\text{00 s} $$
...
m/s}}^{2} $$. Its acceleration is $$ -9\text{.}{\text{80 m/s}}^{2} $$
...
\text{13}\text{.0 m/s} $$. It rises and then falls back down. When its position is $$ y=0 $$
on its way back down, its velocity is $$ -\text{13}\text{.0 m/s} $$. That is, it has the same speed on its way down as on its way up. We would then
...
$$
or thrown it downwards at $$ -\text{13}\text{.0 m/s} $$. The velocity of the rock on its way down from $$ y=0 $$
...
9\text{.}{\text{80 m/s}}^{2} $$, so $$ 9\text{.}{\text{8010 m/s}}^{2} $$
...
, final position $$ y=\text{−30}\text{.}\text{0 m} $$, and $$ a=-g=-9\text{.}{\text{80 m/s}}^{2} $$. We can then use the equation $$ y={y}_{0}+{v}_{0}t+\frac{1}{2}{\text{at}}^{2}
$$
to solve for $$ t $$. Inserting $$ a=-g $$, we obtain

<div data-type="equation" id="import-auto-id2358783">
 $$ \begin{array}{lll}y& =& 0+0-\frac{1}{2}{\text{gt}}^{2}\\ {t}^{2}& =& \frac{2y}{-g}\\ t& =& ±\sqrt{\frac{2y}{-g}}=±\sqrt{\frac{2\left(-\text{30.0 m}\right)}{-9.80 m{\text{/s}}^{2}}}=±\sqrt{\text{6.12}{s}^{2}}=\text{2.47 s}\approx \text{2.5 s}\end{array} $$ 
</div>
