...
Although $$ g $$ varies from $$ 9.78 \text{m/s}^{2} $$ to $$ $$ $$ 9.83
\text{m/s}^{2} $$, depending on latitude, altitude, underlying geological
formations, and local topography, the average value of $$ 9.80 \text{m/s}^{2} $$
...
\text{m/s}^{2} $$, and if we define the downward direction as positive, then $$
a=g=9.80 \text{m/s}^{2} $$.
...
<strong>Solution for Position </strong>  $$ y_{1} $$ 1\. Identify the knowns. We
know that $$ y_{0}=0 $$ ; $$ v_{0}=13.0 \text{m/s} $$ ; $$ a=-g=-9.80
\text{m/s}^{2} $$ ; and $$ t=1.00 \text{s} $$.

2\. Identify the best equation to use. We will use $$ y=y_{0}+v_{0}t+\frac{1}{2}
at^{2} $$ because it includes only one unknown, $$ y $$
...
<strong>Solution for Velocity </strong>  $$ v_{1} $$ 1\. Identify the knowns. We
know that $$ y_{0}=0 $$ ; $$ v_{0}=13.0 \text{m/s} $$ ; $$ a=-g=-9.80
\text{m/s}^{2} $$ ; and $$ t=1.00 \text{s} $$. We also know from the solution
above that $$ y_{1}=8.10 \text{m} $$.
...
at $$ t=1.00\text{s} $$. However, it has slowed from its original 13.0** m/s, as
expected.

**Solution for Remaining Times**

The procedures for calculating the position and velocity at $$ t=2.00\text{s} $$
and $$ 3.00 \text{s} $$ are the same as those above. The results are summarized
in [\[link\]](#eip-304) and illustrated in [\[link\]](#import-auto-id4064055).

<span data-type="title">**Results**</span>


|Time, t|Position, y|Velocity, v|Acceleration, a|
|--- |--- |--- |--- |
| $$1.00 \text{s} $$ | $$8.10 \text{m}$$ | $$3.20 \text{m/s}$$ | $$-9.80 \text{m/s}^2$$ |
| $$2.00 \text{s} $$ | $$6.40 \text{m}$$| $$-6.60 \text{m/s}$$ | $$-9.80 \text{m/s}^2$$ |
| $$3.00 \text{s} $$ | $$-5.10 \text{m}$$ | $$-16.4 \text{m/s}$$ | $$-9.80 \text{m/s}^2$$|
{: #eip-304}
...
\text{m/s}^{2} $$. Its acceleration is $$ -9.80 \text{m/s}^{2} $$
<sup /> for the whole trip—while it is moving up and while it is moving down.
Note that the values for $$ y $$
...
1\. Identify the knowns. $$ y_{0}=0 $$ ; $$ y_{1}=-5.10 \text{m} $$ ; $$ v_
{0}=-13.0 \text{m/s} $$ ; $$ a=-g=-9.80 \text{m/s}^{2} $$.
...
thrown up with an initial velocity of $$ 13.0 \text{m/s} $$. It rises and then
falls back down. When its position is $$ y=0 $$ on its way back down, its
velocity is $$ -13.0 \text{m/s} $$. That is, it has the same speed on its way
down as on its way up. We would then expect its velocity at a position of $$
y=-5.10 \text{m} $$ to be the same whether we have thrown it upwards at $$ +13.0
\text{m/s} $$ or thrown it downwards at $$ -13.0 \text{m/s} $$. The velocity of
...
1\. Identify the knowns. $$ y_{0}=0 $$ ; $$ y=–1.0000 \text{m} $$ ; $$ t=0.45173
\text{s} $$ ; $$ v_{0}=0 $$.
...
value of $$ 9.80 \text{m/s}^{2} $$, so $$ 9.8010 \text{m/s}^{2} $$ makes sense.
Since the data going into the calculation are relatively precise, this value for
$$ g $$ is more precise than the average value of $$ 9.80 \text{m/s}^{2} $$ ; it
...
, final position $$ y=−30.0 \text{m} $$, and $$ a=-g=-9.80 \text{m/s}^{2} $$. We
can then use the equation $$ y=y_{0}+v_{0}t+\frac{1}{2}a t^{2} $$ to solve for
$$ t $$. Inserting $$ a=-g $$, we obtain
...
* {: #import-auto-id2150922} Whether the acceleration <em>a </em>should be taken
  ...
  you choose the upward direction as positive, $$ a=-g=-9.80 \text{m/s}^{2} $$
  is negative. In the opposite case, $$ a=+g=9.80 \text{m/s}^{2} $$ is positive.
...
; $$ v_{1}=10.1 \text{m/s} $$
(b)  $$ y_{2}=10.1 \text{m} $$ ; $$ v_{2}=5.20 \text{m/s} $$
(c)  $$ y_{3}=11.5 m $$ ; $$ v_{3}=0.300 \text{m/s} $$
(d)  $$ y_{4}=10.4 \text{m} $$ ; $$ v_{4}=-4.60 \text{m/s} $$
</div>
</div>
...
; $$ v_{0}=13.0 \text{m/s} $$ ; $$ y_{0}=0 \text{m} $$
(b)  $$ v=0\text{m/s} $$. Unknown is distance $$ y $$ to top of trajectory,
