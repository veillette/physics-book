...
of energy—mechanical energy $$ \left(\text{KE}+\text{PE}\right) $$ and energy
...
conservation of energy. Kinetic energy is $$ \text{KE} $$, work done by a
conservative force is represented by $$ \text{PE} $$, work done by
...
final point, so that $$ {\text{PE}}_{\text{g}} $$ is zero there. Then solve for
...
* {: #import-auto-id1839690} When all forms of energy are considered,
  ...
  {f}+{\text{PE}}_{f}+{\text{OE}}_{f} $$ , where $$ \text{OE} $$ is all **other
...
* {: #import-auto-id1628394} The efficiency $$ \text{Eff} $$ of a machine or
  ...
  $$ , where $$ W_{\text{out}} $$
  <sub /> is useful work output and $$ E_{\text{in}} $$
...
and $$ \text{ΔKE} $$, we obtain $$ v=\sqrt{2g h +{ v_{0}}^{2}}=\sqrt{2\left(9.80
\text{m/s}^{2}\right)\left(20.0 \text{m}\right)+\left(15.0 \text{m/s}{\right)
}^{2}}=24.8 \text{m/s} $$
</div>
</div>
