...
The work going into mechanical energy is $$ W=\text{KE} + \text{PE} $$. At the bottom of the stairs, we take both $$ \text{KE} $$
and $$ \text{PE}_{g} $$ as initially zero; thus, $$ W=\text{KE}_{f}+\text{PE}_{g}=\frac{1}{2} m v_{f}^{2}+mgh $$, where $$ h $$
...
1 **horsepower**{: data-type="term" #import-auto-id1522337} $$ \left(1 \text{hp}=746 \text{W}\right) $$
...
megawatts; 1 megawatt (MW) is $$ 10^{6}\text{W} $$
...
Electricity bills state the energy used in units of **kilowatt-hours**{: data-type="term" #import-auto-id1733623}
 $$
...
, $$ \text{slope angle}\theta =2.00^\circ $$, $$ v=3.00 m/s $$, $$ f=600 \text{N} $$
...
$$ P=\frac{W}{t}=\frac{F d }{t}=F\left(\frac{d}{t}\right)=\text{Fv}, $$
where $$ F $$
...
\sin{2^\circ }\right]
\left(30.0 \text{m/s}\right)\\ & =&
2.77\times 10^{4} \text{W} \end{array} $$
