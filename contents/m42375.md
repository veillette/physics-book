...
-field, such as shown in [\[link\]](#import-auto-id1898240). (If this takes place in a vacuum, the magnetic field is the dominant factor determining the motion.) Here, the magnetic force supplies the centripetal force  $${F}_{c}=mv^{2}/r $$
. Noting that  $$\sin \theta =1 $$
, we see that  $$F=qvB $$
.
...
* Magnetic force can supply centripetal force and cause a charged particle to move in a circular path of radius
  ...
  where
   $$v $$ is the component of the velocity perpendicular to
