...
where  $$d $$  is the distance from A to B, or the distance between the plates in [\[link\]](#import-auto-id671952). Note that the above equation implies the units for electric field are volts per meter. We already know the units for electric field are newtons per coulomb; thus the following relation among units is valid:
