...
. Units for linear momentum are \\( \text{kg}\cdot \text{m}\text{/s} \\)

while units for angular momentum are \\( \text{kg}\cdot {\text{m}}^{2}\text{/s}
\\)

. As we would expect, an object that has a large moment of inertia \\( I \\)

, such as Earth, has a very large angular momentum. An object that has a large
...
. First, according to [\[link\]](/contents/m42179#fs-id1838666), the formula for
...
torque, the more rapid the increase in \\( L \\)
...
linear momentum, \\( F=\text{Δ}p/\text{Δ}t \\)

. The equation \\( \text{net}\phantom{\rule{0.25em}{0ex}}\tau
...
from rest. That is, \\( \text{Δ}L=L \\)

. To find the final velocity, we must calculate \\( \omega \\)
...
, we see that \\( \text{net}\phantom{\rule{0.25em}{0ex}}\tau =\text{rF} \\)
...
second law, or \\( \alpha =\text{net}\phantom{\rule{0.25em}{0ex}}\tau /I \\)
...
\text{net}\phantom{\rule{0.25em}{0ex}}\tau \right)\text{Δ}t \\)

. This equation means that, to change angular momentum, a torque must act over
