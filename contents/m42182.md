...
momentum**{: data-type="term" #import-auto-id3209780} $$ L $$
as
...
p=\text{mv} $$. Units for linear momentum are $$ \text{kg}\cdot \text{m}\text{/s} $$
while units for angular momentum are $$ \text{kg}\cdot {\text{m}}^{2}\text{/s}
$$. As we would expect, an object that has a large moment of inertia $$ I $$, such as Earth, has a very large angular momentum. An object that has a large
angular velocity $$ \omega $$, such as a centrifuge, also has a rather large angular momentum.
...
pertinent data before we can calculate $$
L=\mathrm{I\omega } $$. First, according to [\[link\]](/contents/m42179#fs-id1838666), the formula for
...
<div data-type="equation" id="eip-239">
 $$ L=\mathrm{I\omega }=\frac{2{\text{MR}}^{2}\omega }{5}. $$ 
</div>
Earth’s mass  $$ M $$ 

is $$
5\text{.}\text{979}×{\text{10}}^{\text{24}}\text{kg}
$$
and its radius $$ R $$
is $$ 6\text{.}\text{376}×{\text{10}}^{6}\text{m}
$$. The Earth’s angular velocity $$ \omega $$
is, of course, exactly one revolution per day, but we must covert $$ \omega $$
to radians per second to do the calculation in SI units.

**Solution**

Substituting known information into the expression for $$ L $$
and converting $$ \omega $$
to radians per second gives

<div data-type="equation" id="eip-297">
 $$ \begin{array}{lll}L& =& 0\text{.}4\left(5\text{.}\text{979}×{\text{10}}^{\text{24}}\text{kg}\right){\left(6\text{.}\text{376}×{\text{10}}^{6}\text{m}\right)}^{2}\left(\frac{1\text{rev}}{\text{d}}\right)\\ & =& 9\text{.}\text{72}×{\text{10}}^{\text{37}}\text{kg}\cdot {\text{m}}^{2}\cdot \text{rev/d}.\end{array} $$ 
</div>
Substituting  $$ 2\pi  $$ 

rad for $$ 1 $$
rev and $$
8\text{.}\text{64}×{\text{10}}^{4}\text{s} $$
for 1 day gives

<div data-type="equation" id="eip-195">
 $$ \begin{array}{lll}L& =& \left(9\text{.}\text{72}×{\text{10}}^{\text{37}}\text{kg}\cdot {\text{m}}^{2}\right)\left(\frac{2\pi \text{rad/rev}}{8\text{.}\text{64}×{\text{10}}^{4}\text{s/d}}\right)\left(1\text{rev/d}\right)\\ & =& 7\text{.}\text{07}×{\text{10}}^{\text{33}}\text{kg}\cdot {\text{m}}^{2}\text{/s}.\end{array} $$ 
</div>
**Discussion**
...
torque, the more rapid the increase in $$ L $$. The relationship between torque and angular momentum is

<div data-type="equation" id="eip-628">
 $$ \text{net}\tau =\frac{\text{Δ}L}{\text{Δ}t}. $$ 
</div>

This expression is exactly analogous to the relationship between force and
linear momentum, $$ F=\text{Δ}p/\text{Δ}t $$. The equation $$ \text{net}\tau
=\frac{\text{Δ}L}{\text{Δ}t} $$
is very fundamental and broadly applicable. It is, in fact, the rotational form
...
\text{net}\tau =\frac{\text{Δ}L}{\text{Δ}t} $$
for $$ \text{Δ}L $$, and using the given information to calculate the torque. The final angular
momentum equals the change in angular momentum, because the lazy Susan starts
from rest. That is, $$ \text{Δ}L=L $$. To find the final velocity, we must calculate $$ \omega $$
...
<div data-type="equation" id="eip-994">
 $$ \text{Δ}L=\left(\text{net}\text{τ}\right)\mathrm{\Delta t}. $$ 
</div>
Because the force is perpendicular to  $$ r $$ 

, we see that $$ \text{net}\tau =\text{rF} $$, so that

<div data-type="equation" id="eip-911">
 $$ \begin{array}{lll}L& =& \text{rF}\text{Δ}t=\left(0\text{.}\text{260 m}\right)\left(2.50 N\right)\left(0.150 s\right)\\ & =& 9\text{.}\text{75}×{\text{10}}^{-2}\text{kg}\cdot {\text{m}}^{2}/\text{s}.\end{array} $$ 
</div>
**Solution for (b)**
...
<div data-type="equation" id="eip-963">
 $$ \omega =\frac{L}{I}=\frac{L}{\frac{1}{2}{\mathit{MR}}^{2}}. $$ 
</div>
And substituting known values into the preceding equation yields

<div data-type="equation" id="eip-98">
 $$ \omega =\frac{9\text{.}\text{75}×{\text{10}}^{-2}\text{kg}\cdot {\text{m}}^{2}\text{/s}}{\left(0\text{.}\text{500}\right)\left(4\text{.}\text{00}\text{kg}\right)\left(0\text{.}\text{260}\text{m}\right)}=0\text{.}\text{721}\text{rad/s}. $$ 
</div>
**Discussion**
...
second law, or $$ \alpha =\text{net}\tau /I $$. The moment of inertia $$ I $$
is given and the torque can be found easily from the given force and
perpendicular lever arm. Once the angular acceleration $$ \alpha $$
is known, the final angular velocity and rotational kinetic energy can be
calculated.
...
<div data-type="equation" id="eip-466">
 $$ \begin{array}{lll}\text{net}\tau & =& {r}_{\perp }F\\ & =& \left(0\text{.}\text{0220 m}\right)\left(\text{2000}\text{N}\right)\\ & =& \text{44}\text{.}\text{0 N}\cdot \text{m.}\end{array} $$ 
</div>
...
<div data-type="equation" id="eip-388">
 $$ \alpha =\frac{\text{44}\text{.}0\text{N}\cdot \text{m}}{1\text{.}\text{25}\text{kg}\cdot {\text{m}}^{2}}=\text{35}\text{.}2{\text{rad/s}}^{2}. $$ 
</div>
**Solution to (b)**
...
<div data-type="equation" id="eip-877">
 $$ \begin{array}{lll}{\text{KE}}_{\text{rot}}& =& 0.5\left(1\text{.25}\text{kg}\cdot {\text{m}}^{2}\right)\left(\text{70.}4{\text{rad}}^{2}/{\text{s}}^{2}\right)\\ & =& \text{44}\text{.}0\text{J}\end{array}. $$ 
</div>
**Discussion**
...
example, $$ \text{Δ}L=\left(
\text{net}\tau \right)\text{Δ}t $$. This equation means that, to change angular momentum, a torque must act over
some period of time. Because Earth has a large angular momentum, a large torque
acting over a long time is needed to change its rate of spin. So what external
torques are there? Tidal friction exerts torque that is slowing Earth’s
rotation, but tens of millions of years must pass before the change is very
significant. Recent research indicates the length of the day was 18 h some 900
million years ago. Only the tides exert significant retarding torques on Earth,
and so it will continue to spin, although ever more slowly, for many billions of
years.
...
rigorously by considering $$ \text{net}\tau
=\frac{\text{Δ}L}{\text{Δ}t} $$
...
If the change in angular momentum $$ \text{Δ}L $$
...
exerted very close to the pivot point. (Both $$ F $$
and $$ r $$
are small, and so $$ \tau $$
is negligibly small.) Consequently, she can spin for quite some time. She can do
something else, too. She can increase her rate of spin by pulling her arms and
legs in. Why does pulling her arms and legs in increase her rate of spin? The
answer is that her angular momentum is constant, so that
...
and reduced her moment of inertia. Because $$ I\prime $$
is smaller, the angular velocity $$ \omega \prime $$
must increase to keep the angular momentum constant. The change can be dramatic,
...
Suppose an ice skater, such as the one in [\[link\]](#import-auto-id2452786), is spinning at 0.800 rev/ s with her arms extended. She has a moment of inertia of  $$ 2\text{.}\text{34}\text{kg}\cdot {\text{m}}^{2} $$ 

with her arms extended and of $$
0\text{.}\text{363}\text{kg}\cdot {\text{m}}^{2} $$
with her arms close to her body. (These moments of inertia are based on
reasonable assumptions about a 60.0-kg skater.) (a) What is her angular velocity
in revolutions per second after she pulls in her arms? (b) What is her
rotational kinetic energy before and after she does this?

**Strategy**

In the first part of the problem, we are looking for the skater’s angular
velocity $$ \omega \prime $$
after she has pulled in her arms. To find this quantity, we use the conservation
of angular momentum and note that the moments of inertia and initial angular
velocity are given. To find the initial and final kinetic energies, we use the
...
momentum given in $$ \mathrm{I\omega }=I\prime \omega \prime $$
is applicable. Thus,
...
<div data-type="equation" id="eip-8">
 $$ \begin{array}{lll}{\text{KE}}_{\text{rot}}& =& \left(0\text{.}5\right)\left(2\text{.}\text{34}\text{kg}\cdot {\text{m}}^{2}\right){\left(\left(0\text{.}\text{800}\text{rev/s}\right)\left(2\pi \text{rad/rev}\right)\right)}^{2}\\ & =& 29.6\text{J.}\end{array} $$ 
</div>
...
<div data-type="equation" id="eip-307">
 $$ \begin{array}{lll}K{E}_{\text{rot}}\prime & =& \left(0\text{.}5\right)\left(0\text{.363 kg}\cdot {m}^{2}\right){\left[\left(5\text{.}\text{16 rev/s}\right)\left(2\pi  rad/rev\right)\right]}^{2}\\ & =& \text{191 J.}\end{array} $$ 
</div>
**Discussion**
...
* {: #import-auto-id1381663} Every rotational phenomenon has a direct
  translational analog , likewise angular momentum $$ L $$
  can be defined as $$ L=\mathrm{I\omega }. $$
* {: #import-auto-id2625259} This equation is an analog to the definition of
  linear momentum as $$ p=\text{mv} $$
...
(a)  $$ 2\text{.}\text{66}×{\text{10}}^{\text{40}}\text{kg}\cdot {\text{m}}^{2}\text{/s} $$ 

(b)  $$
7\text{.}\text{07}×{\text{10}}^{\text{33}}\text{kg}\cdot
{\text{m}}^{2}\text{/s} $$
The angular momentum of the Earth in its orbit around the Sun is $$
3\text{.}\text{77}×{\text{10}}^{6} $$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) Calculate the angular momentum of an ice skater spinning at 6.00 rev/s given his moment of inertia is  $$ 0\text{.}\text{400}\text{kg}\cdot {\text{m}}^{2} $$ 
...
angular momentum
: the product of moment of inertia and angular velocity {: #import-auto-id3381111}

law of conservation of angular momentum
: angular momentum is conserved, i.e., the initial angular momentum is equal to
the final angular momentum when no external torque is applied to the system {: #import-auto-id3381114}
