...
Note that Planck’s constant  $$h $$
 ...
 being emitted by a blackbody, for example, the difference between energy levels is only  $$\Delta E=\text{hf}=\left( 6.63 \times 10^{-34} \text{J·s}\right)\left( 10^{14} \text{Hz}\right)= 6.63 \times 10^{-20} \text{J,} $$
