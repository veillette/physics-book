...
<div data-type="equation" id="eip-183">
 $${F}_{2}=\frac{ {A}_{2}}{ {A}_{1}}{F}_{1}=\frac{ {\mathrm{\pi r}}_{2}^{2}}{ {\mathrm{\pi r}}_{1}^{2}}{F}_{1}=\frac{ {\left(1.25 cm\right)}^{2}}{ {\left(0.250 cm\right)}^{2}}×500 \text{N}= 1.25 \times 10^{4}  \text{N}. $$
</div>
**Discussion**
...
(b) If the system is not moving, friction would not play a role. With friction, we know there are losses, so that  $${W}_{\text{out}}={W}_{\text{in}}-{W}_{\text{f}} $$
