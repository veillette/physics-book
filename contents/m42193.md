...
$${F}_{2}=\frac{ {A}_{2}}{ {A}_{1}}{F}_{1}=\frac{ {\mathrm{\pi r}}_{2}^{2}}{ {\mathrm{\pi r}}_{1}^{2}}{F}_{1}=\frac{ {\left(1.25 cm\right)}^{2}}{ {\left(0.250 cm\right)}^{2}}×\text{500 N}=1\text{.}\text{25}×{\text{10}}^{4} \text{N}.$$
</div>
**Discussion**

This value is the force exerted by each of the four slave cylinders. Note that we can add as many slave cylinders as we wish. If each has a 2.50-cm diameter, each will exert $$1\text{.}\text{25}×{\text{10}}^{4} \text{N}\text{.}$$
</div>
