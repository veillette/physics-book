...
* {: #import-auto-id1655645} To solve problems involving Newton’s laws of
  motion, follow the procedure described:
    ...
    3. {: #import-auto-id2638784} Write Newton’s second law in the horizontal
       ...
       . If the object does accelerate in that direction, $$  F_       {\text{net}x}=ma  $$
       .
...
3.    $$ \sum F\text{=+}F-w=ma \text{,} $$
so that $$ F=ma +w=ma +mg =m\left(a+g\right) $$.
      $$ F=\left(70.0 \text{kg}\right) \left\[ \left(39.2 \text{m/s}^{2}\right)
      +\left(9.80 \text{m/s}^{2}\right)\right\] $$
$$ =3.43\times 10^{3}\text{N} $$. The force exerted by the high-jumper is actually down on the ground, but 
      $$ F $$ is up from the ground and makes him jump.

4. This result is reasonable, since it is quite possible for a person to exert a
   force of the magnitude of $$ 10^{3}\text{N} $$. {: type="1" .stepwise}
...
give $$ {\mathbf{\text{F}}}_{\text{tot}} $$. Draw these to scale on the same drawing used in part (b) or a similar picture.
...
3. The tension is given as $$ T=\text{25.0 N.} $$
   ...
   $$  F_{\text{app}}=2T\sin{\theta}=2\left(25.0 \text{N}\right)
   \sin\left(15^\circ\right)=12.9 \text{N} $$
