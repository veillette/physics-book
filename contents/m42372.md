...
—in terms of the force on a charged particle moving in a magnetic field. The SI unit for magnetic field strength  $$B $$
 is called the **tesla**{: data-type="term"} (T) after the eccentric but brilliant inventor Nikola Tesla (1856–1943). To determine how the tesla relates to other SI units, we solve  $$F=\text{qvB} \sin \theta  $$
 for  $$B $$
.

<div data-type="equation" id="import-auto-id1536271">
 $$B=\frac{F}{\text{qv} \sin \theta } $$
</div>
...
We are given the charge, its velocity, and the magnetic field strength and direction. We can thus use the equation  $$F=\text{qvB} \sin \theta  $$
 to find the force.
