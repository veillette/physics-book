...
where $$ W $$ is work, $$ \mathbf{d} $$ is the displacement of the system, and
$$ \theta $$ is the angle between the force vector $$ \mathbf{F} $$ and the
displacement vector $$ \mathbf{d} $$, as in [\[link\]](#import-auto-id2495459).
...
#import-auto-id2681417} (J), and $$ 1\text{J}=1\text{N}\cdot
\text{m}=1\text{kg}\cdot \text{m}^{2}{\text{/s}}^{2} $$. One joule is not a
...
<div data-type="equation">
 $$ \begin{array}{lll}W& =& \left(75.0 \text{N}\right)\left(25.0 \text{m}\right)\cos\left(35.0^\circ\right)\\ & =& 1536 \text{J}=1.54\times 10^{3}\text{J} .
\end{array} $$ 
</div>
...
* Work is the transfer of energy by a force acting on an object as it is
  displaced.
* The work $$ W $$ that a force $$ \mathbf{F} $$ does on an object is the
