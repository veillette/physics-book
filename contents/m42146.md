...
displacement downward. This makes $$ \theta =180}\text{º} $$, and $$ \text{cos 180}\text{º}=–1 $$
; therefore, $$ W $$
is negative.
...
<div data-type="equation">
 $$ \begin{array}{lll}W& =& \left(75.0 \text{N}\right)\left(\text{25.0 m}\right)\cos\left(\text{35.0º}\right)\\ & =& 1536 \text{J}=1.54×10^{3}\text{J.}\end{array} $$ 
</div>
...
<div data-type="equation">
 $$ \frac{W}{2400}\text{kcal}}=1.53×10^{-4}\text{.} $$ 
</div>
**Discussion**
...
* The SI unit for work and energy is the joule (J), where $$
  1\text{J}=1\text{N}\cdot
  \text{m}=\text{1 kg}\cdot {\text{m}}^{2}{\text{/s}}^{2} $$
  .
