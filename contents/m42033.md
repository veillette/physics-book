...
{\text{f}}=3\text{.}5\phantom{\rule{0.25em}{0ex}}\text{m} \\)
...
and his final position is \\( {x}_{f}=2\text{.}0 m \\)
...
(a) The rider’s displacement is \\( \Delta x={x}_{f}-{x}_{0}=\text{−1 km} \\)

. (The displacement is negative because we take east to be positive and west to
be negative.)
