...
Displacement is the *change in position* of an object: <div data-type="equation" id="eip-458">  $$ \Delta x={x}_{\text{f}}-{x}_{0},$ </div> where $\Delta x$ is displacement, ${x}_{\text{f}}$ is the final position, and **${x}_{0} $$  is the initial position.
</div>
...
{\text{f}}=3.5\text{m} $$. Thus her displacement is
...
$$ {x}_{0}=6.0 m $$
and his final position is $$ {x}_{f}=2.0 m $$, so his displacement is

<div data-type="equation" id="eip-778">
 $$ \Delta x={x}_{f}-{x}_{0}=2.0 m-6.0 m=-4.0 m. $$ 
</div>
...
<div data-type="exercise" data-element-type="conceptual-questions">
<div data-type="problem" markdown="1">
Bacteria move back and forth by using their flagella (structures that look like little tails). Speeds of up to  $$ \text{50 μm/s}\left(50 \times 10^{-6}\text{m/s}\right) $$ 
