...
The lowest frequency, called the **fundamental frequency**{: data-type="term" #import-auto-id2392024}, is thus for the longest wavelength, which is seen to be $${\lambda }_{1}=2L$$

. Therefore, the fundamental frequency is $${f}_{1}={v}_{\text{w}}/{\lambda }_{1}={v}_{\text{w}}/2L$$

. In this case, the **overtones**{: data-type="term" #import-auto-id2659292} or harmonics are multiples of the fundamental frequency. As seen in [\[link\]](#import-auto-id3045433), the first harmonic can easily be calculated since $${\lambda }_{2}=L$$
...
$$x=X\phantom{\rule{0.25em}{0ex}}\text{cos}\left(\frac{2\pi \phantom{\rule{0.25em}{0ex}}t}{T}\right)=X\phantom{\rule{0.25em}{0ex}}\text{cos}\left(2\pi \phantom{\rule{0.25em}{0ex}}\text{ft}\right)\text{,}$$
</div>
...
$$x=X\phantom{\rule{0.25em}{0ex}}\text{cos}\left(2\pi \phantom{\rule{0.25em}{0ex}}{f}_{1}t\right)+X\phantom{\rule{0.25em}{0ex}}\text{cos}\left(2\pi \phantom{\rule{0.25em}{0ex}}{f}_{2}t\right)\text{.}$$
</div>

Using a trigonometric identity, it can be shown that

<div data-type="equation" id="eip-810">
$$x=2X\phantom{\rule{0.25em}{0ex}}\text{cos}\left(\pi \phantom{\rule{0.25em}{0ex}}{f}_{\text{B}}t\right)\text{cos}\left(2\pi \phantom{\rule{0.25em}{0ex}}{f}_{\text{ave}}t\right)\text{,}$$
</div>
...
Piano tuners use beats routinely in their work. When comparing a note with a tuning fork, they listen for beats and adjust the string until the beats go away (to zero frequency). For example, if the tuning fork has a $$\text{256}\phantom{\rule{0.25em}{0ex}}\text{Hz}$$

 frequency and two beats per second are heard, then the other frequency is either $$\text{254}$$

 or $$\text{258}\phantom{\rule{0.25em}{0ex}}\text{Hz}$$
