...
divide the right-hand side of the equation by \\( r \\)
...
Now, we solve one of the rotational kinematics equations for \\( \text{αθ} \\)
...
. Rotational kinetic energy has important effects. Flywheels, for example, can
...
, which is converted entirely to \\( \text{KE} \\)

, provided each rolls without slipping. \\( \text{KE} \\)

, however, can take the form of \\( {\text{KE}}_{\text{trans}} \\)
...
and \\( v=\left(2\text{gh}{\right)}^{1/2} \\)

, which is 22% greater than \\( \left(4\text{gh}/3{\right)}^{1/2} \\)
...
, and the net force she exerts is 750 N at an effective perpendicular lever arm
