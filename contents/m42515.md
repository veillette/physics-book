...
<div data-type="equation" id="eip-635">
 $$D\sin \theta =\mathrm{m\lambda }, \text{for} m=1 \text{,} \text{–1,} 2 \text{,} \text{–2,} 3 \text{,} \dots  \text{(destructive),} $$
</div>
...
We are given that  $$\lambda =550 \text{nm} $$
,  $$m=2 $$
...
<div data-type="equation" id="eip-6">
 $$\begin{array}{lll}D& =& \frac{\mathrm{m\lambda }}{ \sin {\theta }_{2}}=\frac{2\left(550 \text{nm}\right)}{\text{sin 45.0º}}\\ & =& \frac{1100 \times 10^{-9} }{0.707}\\ & =&  1.56 \times 10^{-6} .\end{array} $$
</div>
**Solution for (b)**
...
* {: #import-auto-id1169738249784} There is destructive interference for a single slit when
   $$D \sin \theta =\text{mλ}, \text{(for} m=1 \text{,} \text{–1,} 2 \text{,} \text{–2,} 3 \text{,} \dots \right) $$
    , where
   $$D $$
    is the slit width,
   $$\lambda  $$
