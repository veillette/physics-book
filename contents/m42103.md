...
and the intercept is position at time zero—that is, \\( {x}_{0} \\)

. Substituting these symbols into \\( y=\text{mx}+b \\)
...
. The value of 140 m/s for \\( {v}_{Q} \\)
...
graph increases until \\( t=\text{55 s} \\)
...
, etc.) at which the instantaneous velocity is greatest. (c) At which times is
...
, etc.) at which the acceleration is greatest. (c) At which times is it zero? (
