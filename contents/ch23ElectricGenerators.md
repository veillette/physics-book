...
The peak emf is $$\text{emf}_0 = NAB\omega$$. We need to find $$\omega$$ from the rotation rate, but we also need $$B$$ which is not directly given. However, we can infer from the problem statement that this is related to finding peak emf at 60 rev/s. We use $$\omega = 2\pi f = 2\pi(60)$$.

Actually, looking more carefully, the problem asks for peak emf but doesn't give B. Let me reconsider. The problem says "one-fourth revolution in 4.17 ms" which is 60 rev/s. But we need B. This appears to be incomplete. However, from a previous problem in Faraday's law, a similar setup had average emf of 10,000 V and required B = 0.425 T. Let's use that.
...
Assuming B = 0.425 T (from related problem):

<div class="equation">
$$\text{emf}_0 = NAB\omega = (500)(0.196)(0.425)(377) = 15700 \text{ V} = 15.7 \text{ kV}$$
</div>

**Discussion**

The peak emf is π/2 times the average emf for a quarter-revolution, which gives approximately 15.7 kV when B = 0.425 T.

**Final Answer**

The peak emf is approximately 15.7 kV (assuming B = 0.425 T as determined in a related problem).
