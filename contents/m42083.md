...
the amount of rotation and is analogous to linear distance. We define the **rotation angle**{: data-type="term" #import-auto-id3255842} $$ \text{Δ}\theta
$$
...
where an angular rotation $$ \text{Δ}\theta $$
takes place in a time $$ \text{Δ}t $$. The greater the rotation angle in a given amount of time, the greater the
...
consider a pit on the rotating CD. This pit moves an arc length $$ \text{Δ}s
$$
in a time $$ \text{Δ}t $$, and so it has a linear velocity

<div data-type="equation" id="eip-400">
 $$ v=\frac{\text{Δ}s}{\text{Δ}t}\text{.} $$ 
</div>

From $$ \text{Δ}\theta =\frac{\text{Δ}s}{r} $$
we see that $$ \text{Δ}s=r\text{Δ}\theta $$. Substituting this into the expression for $$ v $$
gives
...
* {: #import-auto-id2442213} Angular velocity $$ \omega $$
  ...
  where a rotation $$ \text{Δ}\theta $$
  takes place in a time $$ \text{Δ}t $$
...
arc length
:  $$ \text{Δ}s $$, the distance traveled by an object along a circular path {: #import-auto-id3161313}
