...
<div data-type="equation" id="eip-264">
 $$\theta_{v}={\tan}^{-1}\left(v_{y}/v_{x}\right)\text{.} $$ 
</div>
...
{0}}\sin{\theta}
$$, where $$ v_{0y} $$
is the initial velocity of 70.0 m/s, and $$ \theta_{0}=75.0^\circ $$
is the initial angle. Thus,

<div data-type="equation" id="eip-677">
 $$ v_{0y}=v_{0}\sin{\theta_{0}}=\left(70.0 \text{m/s}\right)\left(\text{sin 75º}\right)=\text{67.6 m/s.} $$ 
</div>
and  $$ y $$ 
...
is the **x**-component of the velocity, which is given by $$ v_{x}=v_{0}\cos{\theta_{0}}\text{.} $$
Now,

<div data-type="equation" id="eip-884">
 $$ v_{x}=v_{0}\cos{\theta_{0}}=\left(70.0 m/s\text{}\right)\left(\text{cos 75.0º}\right)=18.1 m/s. $$ 
</div>
The time  $$ t $$ 
...
velocity, found from $$ v_{0y}=v_{0}\sin{\theta_{0}} $$
= ( $$ 25.0 \text{m/s} $$
)( $$ \text{sin 35.0º} $$
...
<div data-type="equation" id="eip-873">
 $$ v_{x}=v_{0}\cos{\theta_{0}}=\left(25.0 m/s\text{}\right)\left(\text{cos 35º}\right)=20.5 m/s $$ 
</div>
...
<div data-type="equation" id="eip-589">
 $$\theta_{v}={\tan}^{-1}\left(-24.5/20.5\right)={\tan}^{-1}\left(-1.19\right)\text{.} $$ 
</div>
Thus,
...
might be produced by a cannon, the maximum range is obtained with $$ \theta_{0}=45^\circ $$. This is true only for conditions neglecting air resistance. If air resistance
...
where $$ v_{0} $$
is the initial speed and $$\theta_{0} $$
...
* {: #import-auto-id1823593} The maximum horizontal distance traveled by a
  ...
  of a projectile on level ground launched at an angle $$\theta_{0} $$
