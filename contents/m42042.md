...
notation for displacement, where $$ \mathbf{s} $$ is defined to be the total displacement and $$ \mathbf{x} $$
and $$ \mathbf{y} $$ are its components along the horizontal and vertical axes, respectively. The
magnitudes of these vectors are **s**, **x**, and **y**. (Note that in the last section we used the notation $$
\mathbf{A} $$ to represent a vector with components $$ {\mathbf{A}}_{x} $$
and $$ {\mathbf{A}}_{y} $$. If we continued this format, we would call displacement $$ \mathbf{s} $$
with components $$ {\mathbf{s}}_{x} $$ and $$ {\mathbf{s}}_{y} $$. However, to simplify the notation, we will simply represent the component
vectors as $$ \mathbf{x} $$ and $$ \mathbf{y} $$.)
...
then very simple:  $$ a_{y}=–g=–9.80 m{\text{/s}}^{2} $$. (Note that this definition assumes that the upwards direction is defined as
...
positive value.) Because gravity is vertical, $$ a_{x}=0 $$. Both accelerations are constant, so the kinematic equations can be used.
...
components along the x- and y-axes.* These axes are perpendicular, so $$ A_{x}=A\cos{\theta} $$
and $$  A_{y}=A\sin{\theta} $$ are used. The magnitude of the components of displacement $$ \mathbf{s} $$
along these axes are $$ x $$ and $$ y$$.
The magnitudes of the components of the velocity $$ \mathbf{v} $$
are $$ v_{x}=v\cos{\theta} $$ and $$ v_{y}=v\sin{\theta}$$, where $$ v $$ is the magnitude of the velocity and $$ \theta $$
...
<div data-type="equation" id="eip-293">
 $$ \text{Vertical Motion}\left(\text{assuming positive is up}a_{y}=-g=-9.80{\text{m/s}}^{2}\right) $$ 
</div>

<div data-type="equation" id="eip-131">
 $$ y={y}_{0}+\frac{1}{2}\left(v_{0y}+v_{y}\right)t $$ 
</div>
...
<div data-type="equation" id="eip-243">
 $$ v_{y}^{2}= v_{0y}^{2}-2g \left( y-{y}_{0} \right) \text{.} $$ 
</div>
...
employing $$ A=\sqrt{ A_{x}^{2}+ A_
{y}^{2}} $$
and $$ \theta ={\tan}^{-1}\left( A_{y}/ A_{x}\right) $$
...
<div data-type="equation" id="eip-264">
 $$ {\theta }_{v}={\tan}^{-1}\left(v_{y}/v_{x}\right)\text{.} $$ 
</div>
...
vertical motions in which $$ a_{x}=0 $$
and $$ a_{y}=–g $$. We can then define $$ {x}_{0} $$
and $$ {y}_{0} $$
...
is reached when $$ v_{y}=0 $$. Since we know the initial and final velocities as well as the initial
position, we use the following equation to find $$ y $$
\:

<div data-type="equation" id="eip-734">
 $$ v_{y}^{2}={v}_{0y}^{2}-2g\left(y-{y}_{0}\right)\text{.} $$ 
</div>
...
<div data-type="equation" id="eip-677">
 $$ {v}_{0y}={v}_{0}\sin{\theta }_{0}=\left(\text{70.0 m/s}\right)\left(\text{sin 75º}\right)=\text{67.6 m/s.} $$ 
</div>
and  $$ y $$ 
...
<div data-type="equation" id="eip-512">
 $$ y=\frac{\left(67}\text{.6 m/s}{\right)}^{2}}{2\left(9.80 \text{m}{\text{/s}}^{2}\right)}, $$ 
</div>
so that
...
{0}+\frac{1}{2}\left({v}_{0y}+v_{y}\right)t $$. Because $$ {y}_{0} $$
is zero, this equation reduces to simply

<div data-type="equation" id="eip-383">
 $$ y=\frac{1}{2}\left({v}_{0y}+v_{y}\right)t\text{.} $$ 
</div>
Note that the final vertical velocity,  $$ v_{y} $$ 

, at the highest point is zero. Thus,

<div data-type="equation" id="eip-50">
 $$ \begin{array}{lll}t& =& \frac{2y}{\left({v}_{0y}+v_{y}\right)}=\frac{2\left(\text{233 m}\right)}{\left(\text{67.6 m/s}\right)}\\ & =& \text{6.90 s}\text{.}\end{array} $$ 
</div>
**Discussion for (b)**
...
Because air resistance is negligible, $$ a_{x}=0 $$
...
is the **x**-component of the velocity, which is given by $$ v_{x}={v}_
{0}\cos{\theta }_
{0}\text{.} $$
Now,

<div data-type="equation" id="eip-884">
 $$ v_{x}={v}_{0}\cos{\theta }_{0}=\left(70.0 m/s\text{}\right)\left(\text{cos 75.0º}\right)=18.1 m/s. $$ 
</div>
The time  $$ t $$ 
...
<div data-type="equation" id="eip-685">
 $$ x=\left(18.1 m/s\text{}\right)\left(6.\text{90 s}\text{}\right)=\text{125 m.}\text{} $$ 
</div>
**Discussion for (c)**
...
to be zero, then the final position is $$ y=-20}\text{.0 m}\text{.} $$
...
) = $$ 14.3 \text{m/s} $$. Substituting known values yields

<div data-type="equation" id="eip-722">
 $$ -20.0 m\text{}=\left(14.3 m/s\text{}\right)t-\left(4.90 \text{m/s}{\text{}}^{2}\right){t}^{2}\text{.} $$ 
</div>
...
<div data-type="equation" id="eip-931">
 $$ \left(4.90 \text{m/s}{\text{}}^{2}\right){t}^{2}-\left(14.3 \text{m/s}\right)t-\left(\text{20.0 m}\right)=0. $$ 
</div>
...
<div data-type="equation" id="eip-880">
 $$ t=\frac{-b±\sqrt{ b^{2}-4\text{ac}}}{2}\text{a}}\text{.} $$ 
</div>
...
is $$
t=3.96\text{s} $$
or $$ –1.03\text{s} $$. The negative value of time implies an event before the start of motion, and so
we discard it. Thus,
...
it makes with the horizontal. Of course, $$ v_{x} $$
...
<div data-type="equation" id="eip-873">
 $$ v_{x}={v}_{0}\cos{\theta }_{0}=\left(25.0 m/s\text{}\right)\left(\text{cos 35º}\right)=20.5 m/s $$ 
</div>
...
was found in part (a) to be $$ 14.3 \text{m/s} $$. Thus,

<div data-type="equation" id="eip-113">
 $$ v_{y}=14.3 m/s\text{}-\left(9.80 \text{m/s}{\text{}}^{2}\right)\left(3.96 \text{s}\text{}\right) $$ 
</div>
so that
...
<div data-type="equation" id="eip-394">
 $$ v=\sqrt{ v_{x}^{2}+v_{y}^{2}}=\sqrt{\left(20.5 m/s\text{}{\right)}^{2}+\left(-24.5 m/s\text{}{\right)}^{2}}\text{,} $$ 
</div>
which gives
...
<div data-type="equation" id="eip-353">
 $$ {\theta }_{v}={\tan}^{-1}\left(v_{y}/v_{x}\right) $$ 
</div>
so that

<div data-type="equation" id="eip-589">
 $$ {\theta }_{v}={\tan}^{-1}\left(-24.5/20.5\right)={\tan}^{-1}\left(-1.19\right)\text{.} $$ 
</div>
Thus,
...
The negative angle means that the velocity is $$ 50.1º $$
...
<div data-type="equation" id="eip-240">
 $$ R=\frac{ v_{0}^{2}\sin{2\theta }_{0}}{g}\text{,} $$ 
</div>
  ...
  following steps:
    ...
    2. {: #import-auto-id1830316} Analyze the motion of the projectile in the
       ...
       <div data-type="equation" id="eip-612">
        $$ v_{x}={v}_{0x}={\mathbf{\text{v}}}_{\text{x}}=\text{velocity is a constant.} $$ 
       </div>
    ...
    3. {: #import-auto-id1939082} Analyze the motion of the projectile in the
       ...
       <div data-type="equation" id="import-auto-id1939084">
        $$ \text{Vertical motion}\left(\text{Assuming positive direction is up;}a_{y}=-g=-9.80 \text{m}{\text{/s}}^{2}\right) $$ 
       </div>

       <div data-type="equation" id="import-auto-id1492830">
        $$ y={y}_{0}+\frac{1}{2}\left({v}_{0y}+v_{y}\right)t $$ 
       </div>
       ...
       <div data-type="equation" id="import-auto-id1653540">
        $$ v_{y}^{2}={v}_{0y}^{2}-2g\left(y-{y}_{0}\right). $$ 
       </div>
    ...
    4. {: #import-auto-id1552181} Recombine the horizontal and vertical
       ...
       <div data-type="equation" id="import-auto-id1979208">
        $$ {\theta }_{\text{v}}={\tan}^{-1}\left(v_{y}/v_{x}\right). $$ 
       </div>
  {: type="1" .stepwise}
...
<div data-type="solution" markdown="1">
 $$ \begin{array}{lll}x& =& \text{1.30 m}\times 10^{2}\\ y& =& 30}\text{.9 m.}\end{array} $$ 
...
ramp at a speed of $$ 40.\text{0 m/s }\left(\text{144
...
<div data-type="solution" markdown="1">
 $$ \begin{array}{}R=\frac{ { v_{0}}}{\sin{2\theta }_{0}g}\\ \text{For}\theta =\text{45º},R=\frac{ { v_{0}}}{g}\end{array} $$ 

$$ R=91.8\text{m} $$
for $$ {v}_{0}=30\text{m/s} $$
; $$ R=163\text{m} $$
for $$ {v}_{0}=40\text{m/s} $$
; $$ R=255\text{m} $$
for $$ {v}_{0}=50\text{m/s} $$.
...
<div data-type="solution" id="eip-id2932521" markdown="1">
 $$ y-{y}_{0}=0={v}_{0y}t-\frac{1}{2}{\mathrm{gt}}^{2}=\left({v}_{0}\sin{\theta} \right)t-\frac{1}{2}{\mathrm{gt}}^{2} $$ 
