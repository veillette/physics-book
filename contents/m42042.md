...
then very simple:  $$ a_{y}=–g=–9.80 \text{m/s}^{2} $$. (Note that this definition assumes that the upwards direction is defined as
...
employing $$ A=\sqrt{ A_{x}^{2}+ A_{y}^{2}} $$
...
During a fireworks display, a shell is shot into the air with an initial speed of 70.0 m/s at an angle of  $$ 75.0^\circ $$ 
...
{0}}\sin{\theta}
$$, where $$ v_{0y} $$
is the initial velocity of 70.0 m/s, and $$ {\theta }_{0}=75.0^\circ $$
...
the highest point. In this case, the easiest method is to use $$ y=y_{0}+\frac{1}{2}\left(v_{0y}+v_{y}\right)t $$. Because $$ y_{0} $$
...
displacement is horizontal velocity multiplied by time as given by $$ x=x_{0}+v_{x}t $$, where $$ x_{0} $$
is equal to zero:
...
is the **x**-component of the velocity, which is given by $$ v_{x}=v_{0}\cos{\theta }_{0}\text{.} $$
Now,
...
Kilauea in Hawaii is the world’s most continuously active volcano. Very active volcanoes characteristically eject red-hot rocks and lava rather than smoke and ash. Suppose a large rock is ejected from the volcano with a speed of 25.0 m/s and at an angle  $$ 35.0^\circ $$ 
...
velocity, found from $$ v_{0y}=v_{0}\sin{\theta }_{0} $$
= ( $$ 25.0 \text{m/s} $$
)( $$ \text{sin 35.0º} $$
...
The negative angle means that the velocity is $$ 50.1^\circ $$
...
might be produced by a cannon, the maximum range is obtained with $$ {\theta }_{0}=45^\circ $$. This is true only for conditions neglecting air resistance. If air resistance
is considered, the maximum angle is approximately $$ 38^\circ $$. Interestingly, for every initial angle except $$ 45^\circ $$, there are two angles that give the same range—the sum of those angles is $$
90^\circ $$. The range also depends on the value of the acceleration of gravity $$ g $$. The lunar astronaut Alan Shepherd was able to drive a golf ball a great
  ...
  following steps:
    ...
    3. {: #import-auto-id1939082} Analyze the motion of the projectile in the
       ...
       <div data-type="equation" id="import-auto-id1939084">
        $$ \text{Vertical motion}\left(\text{Assuming positive direction is up;}a_{y}=-g=-9.80 \text{m/s}^{2}\right) $$ 
       </div>
...
Answer the following questions for projectile motion on level ground assuming negligible air resistance (the initial angle being neither  $$ 0^\circ $$ 
...
<div data-type="problem" markdown="1">
A projectile is launched at ground level with an initial speed of 50.0 m/s at an angle of  $$ 30.0^\circ $$ 
...
ramp at a speed of $$ 40.0 \text{ m/s }\left(\text{144
...
<div data-type="solution" markdown="1">
 $$ \begin{array}{}R=\frac{ { v_{0}}}{\sin{2\theta }_{0}g}\\ \text{For}\theta =45^\circ,R=\frac{ { v_{0}}}{g}\end{array} $$ 
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
A football quarterback is moving straight backward at a speed of 2.00 m/s when he throws a pass to a player 18.0 m straight downfield. (a) If the ball is thrown at an angle of  $$ 25^\circ $$ 
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
An owl is carrying a mouse to the chicks in its nest. Its position at that time is 4.00 m west and 12.0 m above the center of the 30.0 cm diameter nest. The owl is flying east at 3.50 m/s at an angle  $$ 30.0^\circ $$ 
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Suppose a soccer player kicks the ball from a distance 30 m toward the goal. Find the initial speed of the ball if it just passes over the goal, 2.4 m above the ground, given the initial direction to be  $$ 40^\circ $$ 
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
In 2007, Michael Carter (U.S.) set a world record in the shot put with a throw of 24.77 m. What was the initial speed of the shot if he released it at a height of 2.10 m and threw it at an angle of  $$ 38.0^\circ $$ 

above the horizontal? (Although the maximum distance for a projectile on level
ground is achieved at $$ 45^\circ $$
when air resistance is neglected, the actual angle to achieve maximum range is
smaller; thus, $$ 38^\circ $$
will give a longer range than $$ 45^\circ $$
in the shot put.)
...
<div data-type="solution" id="eip-id2932521" markdown="1">
 $$ y-y_{0}=0=v_{0y}t-\frac{1}{2}g t^{2}=\left(v_{0}\sin{\theta} \right)t-\frac{1}{2}gt^{2} $$ 
,
so that $$ t=\frac{2\left(v_{0}\sin{\theta}\right)}{g} $$
$$ x-x_{0}=v_{0x}t=\left(v_{0}\cos{\theta}\right)t=R, $$
and substituting for $$ t $$
gives:

$$ R=v_{0}\cos{\theta} \left(\frac{ 2 v_{0}\sin{\theta}}{g}\right)=\frac{ 2v_{0}^{2}\sin{\theta}
\cos{\theta} }{g}
$$
