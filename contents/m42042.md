...
# import-auto-id1846113} of an object thrown or projected into the air, subject to only the acceleration of gravity. The
...
# import-auto-id1230666} **is negligible**.
...
. If we continued this format, we would call displacement \\( \mathbf{s} \\)

with components \\( {\mathbf{s}}_{x} \\)

and \\( {\mathbf{s}}_{y} \\)

. However, to simplify the notation, we will simply represent the component
vectors as \\( \mathbf{x} \\)
...
then very simple:  \\( {a}_{y}=–g=–9.80 m{\text{/s}}^{2} \\)

. (Note that this definition assumes that the upwards direction is defined as
...
positive value.) Because gravity is vertical, \\( {a}_{x}=0 \\)
...
is reached when \\( {v}_{y}=0 \\)

. Since we know the initial and final velocities as well as the initial
...
{0}+\frac{1}{2}\left({v}_{0y}+{v}_{y}\right)t \\)
...
. It is also important to define the positive and negative directions in the \\(
x \\)
...
motion. When this is the case, the vertical acceleration, \\( g \\)

, takes a negative value (since it is directed downwards towards the Earth).
...
, where the constants are \\( a=4.90 \\)
...
. (It is left as an exercise for the reader to verify these solutions.) The time
is \\(
t=3.96\phantom{\rule{0.25em}{0ex}}\text{s} \\)

or \\( –1.03\phantom{\rule{0.25em}{0ex}}\text{s} \\)

. The negative value of time implies an event before the start of motion, and so
we discard it. Thus,
...
was found in part (a) to be \\( \text{14}\text{.}\text{3 m/s} \\)
...
#import-auto-id1751163} to be the horizontal distance \\( R \\)
...
greater the initial speed \\( {v}_{0} \\)

, the greater the range, as shown in [\[link\]](#import-auto-id1904800)(a). The
...
. This is true only for conditions neglecting air resistance. If air resistance
is considered, the maximum angle is approximately \\( \text{38º} \\)

. Interestingly, for every initial angle except \\( \text{45º} \\)
...
. How many buses can he clear if the top of the takeoff ramp is at the same
...
into the expression for \\( x-{x}_{0} \\)

, noting that \\( R=x-{x}_{0} \\)
