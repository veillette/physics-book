...
<div data-type="equation" id="eip-289">
 $$\begin{array}{lll}V& =& \text{Ah}=\left(50.0 {\text{km}}^{2}\right)\left(40.0 \text{m}\right)\\ & =& \left[\left(\text{50.0 k}{\text{m}}^{2}\right){\left(\frac{ 10^{3} \text{m}}{1 \text{km}}\right)}^{2}\right]\left(\text{40.0 m}\right)= 2.00 \times 10^{9}  {\text{m}}^{3}\end{array} $$
</div>
