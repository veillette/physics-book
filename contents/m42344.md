...
[[Figure 1]](#Figure1) shows the schematic for a simple circuit. A **simple circuit** has a single voltage source and a single resistor. The wires connecting the voltage source to the resistor can be assumed to have negligible resistance, or their resistance can be included in  $$R $$.
 ...
 **drop** is often used for this voltage. For instance, the headlight in [[link]](#fs-id3120142) has an  $$IR $$
 ...
 and the same  $$q $$
 flows through each. Thus the energy supplied by the voltage source and the energy converted by the resistor are equal. (See [[Figure 2]](#Figure2).)
