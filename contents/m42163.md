...
by the fact that object 2 is initially at rest, and thus \\( {v}_{2}=0 \\)

. Once we simplify these equations, we combine them algebraically to solve for
the unknowns.
...
7\text{.}\text{50}×{\text{10}}^{3}\phantom{\rule{0.25em}{0ex}}\text{kg} \\)

. If the two satellites collide elastically rather than dock, what is their
