...
\overline{a}=\frac{\Delta v}{\Delta t}=\frac{ v_{f}-v_{0}}{ t_{f}-t_
{0}} $$.
...
-15.0 \text{m/s} $$, its change in velocity equals its final velocity:  $$ \Delta v=v_
{f}=-15}.0 \text{m/s} $$.
...
<div data-type="equation" id="import-auto-id2400983">
 $$ \overline{a}=\frac{\Delta v}{\Delta t}=\frac{-15}.0 \text{m/s}}{1.80 \text{s}}=-8.33 \text{m}{\text{/s}}^{2}. $$ 
</div>
**Discussion**

The negative sign for acceleration indicates that acceleration is toward the
west. An acceleration of $$ 8.33 \text{m}{\text{/s}}^{2} $$
due west means that the horse increases its velocity by 8.33 m/s due west each
second, that is, 8.33 meters per second per second, which we write as $$ 8.33 \text{m}{\text{/s}}^{2} $$. This is truly an average acceleration, because the ride is not smooth. We
...
and $$ \Delta t $$, and solve for $$ \overline{a} $$.
...
<div data-type="equation" id="import-auto-id2581171">
 $$ \overline{a}=\left(\frac{+20.0 \text{km/h}}{10.0 \text{s}}\right)\left(\frac{ 10^{3}m}{1 km}\right)\left(\frac{1 h}{3600 \text{s}}\right)=+0.556 \text{m}{\text{/s}}^{2} $$ 
</div>
**Discussion**
