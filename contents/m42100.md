...
+3.0 \text{m/s}^{2} $$
and $$ \text{–2}.0 m{\text{m/s}}^{2} $$, respectively.
...
<div data-type="equation" id="import-auto-id2581171">
 $$ \overline{a}=\left(\frac{+20.0 \text{km/h}}{10.0 \text{s}}\right)\left(\frac{ 10^{3}m}{1 km}\right)\left(\frac{1 h}{3600 \text{s}}\right)=+0.556 \text{m/s}^{2} $$ 
</div>
**Discussion**
