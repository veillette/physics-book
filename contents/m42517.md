...
$$\begin{array}{}\theta =1\text{.}\text{22}\frac{\text{550}×{\text{10}}^{-9}\phantom{\rule{0.25em}{0ex}}\text{m}}{2\text{.}\text{40 m}}\\ = 2.80×{\text{10}}^{-7}\phantom{\rule{0.25em}{0ex}}\text{rad.}\end{array}$$
</div>
**Solution for (b)**
...
$$\begin{array}{lll}s& =& \left(2.0×{\text{10}}^{6}\phantom{\rule{0.25em}{0ex}}\text{ly}\right)\left(\text{2.80}×{\text{10}}^{-7}\phantom{\rule{0.25em}{0ex}}\text{rad}\right)\\ & =& 0\text{.}\text{56 ly.}\end{array}$$
</div>
**Discussion**
...
. Take, for example, a laser beam made of rays as parallel as possible (angles between rays as close to $$\theta =0º$$
 as possible) instead spreads out at an angle $$\theta =1\text{.}\text{22}\phantom{\rule{0.25em}{0ex}}\lambda /D$$
, where $$D$$
...
), so that $$\text{tan}\phantom{\rule{0.25em}{0ex}}\theta \approx \text{sin}\phantom{\rule{0.25em}{0ex}}\theta \approx \theta $$
.

Therefore, the resolving power is

<div data-type="equation" id="eip-790">
$$x=1\text{.}\text{22}\frac{\mathrm{\lambda d}}{D}\text{.}$$
</div>
...
Another way to look at this is by re-examining the concept of Numerical Aperture ($$\text{NA}$$
 ...
 is a measure of the maximum acceptance angle at which the fiber will take light and still contain it within the fiber. [\[link\]](#import-auto-id1169738145533)(b) shows a lens and an object at point P. The $$\text{NA}$$
...
$$\text{sin}\phantom{\rule{0.25em}{0ex}}\alpha =\frac{D/2}{d}=\frac{D}{2d}\text{.}$$
</div>

The $$\text{NA}$$
 for a lens is $$\text{NA}=n\phantom{\rule{0.25em}{0ex}}\text{sin}\phantom{\rule{0.25em}{0ex}}\alpha $$
, where $$n$$
 is the index of refraction of the medium between the objective lens and the object at point P.

From this definition for $$\text{NA}$$
, we can see that

<div data-type="equation" id="eip-14">
$$x=1\text{.}\text{22}\frac{\mathrm{\lambda d}}{D}=1\text{.}\text{22}\frac{\lambda }{2\phantom{\rule{0.25em}{0ex}}\text{sin}\phantom{\rule{0.25em}{0ex}}\alpha }=\text{0.61}\frac{\mathrm{\lambda n}}{\text{NA}}\text{.}$$
</div>
...
One of the consequences of diffraction is that the focal point of a beam has a finite width and intensity distribution. Consider focusing when only considering geometric optics, shown in [\[link\]](#import-auto-id1169737966965)(a). The focal point is infinitely small with a huge intensity and the capacity to incinerate most samples irrespective of the $$\text{NA}$$
 of the objective lens. For wave optics, due to diffraction, the focal point spreads to become a focal spot (see [\[link\]](#import-auto-id1169737966965)(b)) with the size of the spot decreasing with increasing $$\text{NA}$$
...
(a) $$1\text{.}\text{63}×{\text{10}}^{-4}\phantom{\rule{0.25em}{0ex}}\text{rad}$$
(b) 326 ly
...
<div data-type="solution" markdown="1">
$$1\text{.}\text{46}×{\text{10}}^{-5}\phantom{\rule{0.25em}{0ex}}\text{rad}$$
</div>
</div>
...
(b) Neglecting atmospheric effects, what is the size of the spot this beam would make on the Moon, assuming a lunar distance of $$\text{3.84}×{\text{10}}^{8}\phantom{\rule{0.25em}{0ex}}\text{m}$$
?

</div>
<div data-type="solution" markdown="1">
(a) $$3\text{.}\text{04}×{\text{10}}^{-7}\phantom{\rule{0.25em}{0ex}}\text{rad}$$
(b) Diameter of $$235 m$$
</div>
</div>
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
(a) The planet Pluto and its Moon Charon are separated by 19,600 km. Neglecting atmospheric effects, should the 5.08-m-diameter Mount Palomar telescope be able to resolve these bodies when they are $$4\text{.}\text{50}×{\text{10}}^{9}\phantom{\rule{0.25em}{0ex}}\text{km}$$
...
(a) What diameter mirror is needed to be able to see 1.00 m detail on a Jovian Moon at a distance of $$7\text{.}\text{50}×{\text{10}}^{8}\phantom{\rule{0.25em}{0ex}}\text{km}$$
