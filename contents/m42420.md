...
, will oppose the change. Thus an emf will be induced given by ** $$\text{emf}=-L\left(\Delta I/\Delta t\right)=\left( 1.0 H\right)\left[\left(10 \text{A}\right)/\left( 1.0 ms\right)\right]=\text{10 000 V} $$
...
The cross-sectional area in this example is  $$A={\mathrm{\pi r}}^{2}=\left( 3.14\text{.}\text{.}\text{.}\right)\left(0\text{.0200 m}{\right)}^{2}= 1.26 \times 10^{-3}  {\text{m}}^{2} $$
,  $$N $$
...
<div data-type="equation" id="eip-334">
 $$\begin{array}{lll}L& =& \frac{\left(4\pi  \times 10^{-7}  \text{T}\cdot \text{m/A}\right)\left(\text{200}{\right)}^{2}\left( 1.26 \times 10^{-3}  {\text{m}}^{2}\right)}{0.100 m}\\ & =& 0.632 \text{mH}\text{.}\end{array} $$
</div>
**Discussion**
...
* {: #import-auto-id1169738244239} The self-inductance of a solenoid is
  ...
  where
   ...
   $$\ell  $$
    is its length, and
   $${\text{μ}}_{0}=4\pi  \times 10^{\text{−7}} \text{T}\cdot \text{m/A}  $$
