...
where  $$M $$
 ...
 the more effective the coupling. For example, the coils in [\[link\]](#import-auto-id1169738104251) have a small  $$M $$
...
where  $$L $$
** is the self-inductance of the device. A device that exhibits significant self-inductance is called an **inductor**, and given the symbol in [\[link\]](#import-auto-id1169738117225).
...
Let us use this last equation to find an expression for the inductance of a solenoid. Since the area  $$A $$
 ...
 is the solenoid’s length.) Only the current changes, so that  $$\Delta \Phi =A\Delta B={\mu }_{0}\text{NA}\frac{\Delta I}{\ell } $$ .
 Substituting  $$\Delta \Phi  $$
 into  $$L=N\frac{\Delta \Phi }{\Delta I} $$
 gives

<div class="equation" id="eip-613">
 $$L=N\frac{\Delta \Phi }{\Delta I}=N\frac{ {\mu }_{0}\text{NA}\frac{\Delta I}{\ell }}{\Delta I}\text{.} $$
</div>
...
This is a straightforward application of  $$L=\frac{ {\mu }_{0}{N}^{2}A}{\ell } $$ ,
 since all quantities in the equation except  $$L $$
 are known.
...
<div class="equation" id="eip-334">
 $$\begin{array}{lll}L& =& \frac{\left(4\pi  \times 10^{-7}  \text{T}\cdot \text{m/A}\right)\left(200{\right)}^{2}\left( 1.26 \times 10^{-3}  {\text{m}}^{2}\right)}{0.100 m}\\ & =& 0.632 \text{mH}\text{.}\end{array} $$
</div>
**Discussion**
