...
* Show that the gravitational potential energy of an object of mass
   $$ m $$  at height $$ h $$ on Earth is given by $$ \text{PE}_{\text{g}}=mgh $$.
...
mgh & =& \left(0.500 \text{kg}\right)\left( 9.80\text{m/s}^{2}\right)\left(1.00 \text{m}\right)\\
 & =& 4.90 \text{kg}\cdot {m}^{2}{\text{/s}}^{2}= 4.90 \text{J} .
\end{array} $$ 
</div>
...
<div data-type="equation" id="eip-118">
 $$ F=-\frac{mgh }{d}=-\frac{ \left(60.0 \text{kg}\right)\left(9.80 \text{m/s}^{2}\right)\left(-3.00 \text{m}\right)}{5.00\times 10^{-3}\text{m}}=3.53\times 10^{5}N. $$ 
</div>
...
* {: #import-auto-id1527618} As an object descends without friction, its
  ...
  increasing speed, so that $$ \text{ΔKE}=-\text{ΔPE}_{\text{g}} $$ .
...
than when it started from rest. This implies that  $$ \Delta \text{PE} \gg \text{KE}_{\text{i}} $$ . Confirm this statement by taking the ratio of $$ \Delta \text{PE} $$
...
<div data-type="equation" id="eip-id2183293">
 $$ v_{f}=\sqrt{2g h +{ v_{0}}^{2}}=\sqrt{2\left(9.80 \text{m/s}^{2}\right)\left(-0.180 \text{m}\right)+\left(2.00 \text{m/s} \right)^{2}}=0.687 \text{m/s} $$ 
</div>
</div>
</div>
