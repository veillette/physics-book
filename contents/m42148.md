...
, such as in [\[link\]](#import-auto-id1697782). If the object is lifted
straight up at constant speed, then the force needed to lift it is equal to its
weight \\( \text{mg} \\)

. The work done on the mass is then \\( \text{W = Fd = mgh} \\)

. We define this to be the **gravitational potential energy**{: data-type="term"
} \\( \left({\text{PE}}_
{\text{g}}\right) \\)
...
applies for any path that has a change in height of \\( h \\)

, not just when the mass is lifted straight up. (
...
is accompanied by a change in gravitational potential energy \\( \text{mgh} \\)
...
. Using the equations for \\( {\text{PE}}_{\text{g}} \\)
...
\text{ΔKE}=\frac{1}{2}{\text{mv}}^{2} \\)

. The equation for change in potential energy states that \\( {\text{ΔPE}}_
{\text{g}}=\text{mgh} \\)
...
Again \\( -{\text{ΔPE}}_{\text{g}}=\text{ΔKE} \\)

. In this case there is initial kinetic energy, so \\(
\text{ΔKE}=\frac{1}{2}m{v}^{2}-\frac{1}{2}m{ {v}_{0}}^{2} \\)
