...
Ultrasound, like any wave, carries energy that can be absorbed by the medium carrying it, producing effects that vary with intensity. When focused to intensities of $${\text{10}}^{\text{3}}$$
...
Most of the energy carried by high-intensity ultrasound in tissue is converted to thermal energy. In fact, intensities of $${\text{10}}^{3}$$

 to $${\text{10}}^{4}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
...
Reflections at boundaries between two different media occur because of differences in a characteristic known as the <strong data-type="term" id="import-auto-id3007118">acoustic impedance </strong>*$$Z$$
...
where $$\rho $$
 ...
 is the speed of sound through the medium (in m/s). The units for $$Z$$

 are therefore $${kg/\left(m}^{2}·\text{s)}$$
...
The **intensity reflection coefficient**{: data-type="term"} $$a$$
...
(a) Using the values for density and the speed of ultrasound given in [\[link\]](#import-auto-id2672118), show that the acoustic impedance of fat tissue is indeed $$1.34×{10}^{6}\phantom{\rule{0.25em}{0ex}}{kg/\left(m}^{2}·s\right)$$
...
$$Z=\mathrm{\rho v}=\left(\text{925 kg}{\text{/m}}^{3}\right)\left(\text{1450 m/s}\right)$$
</div>
(2) Calculate to find the acoustic impedance of fat tissue.

<div data-type="equation" id="eip-626">
$$1.34×{10}^{6}\phantom{\rule{0.25em}{0ex}}{kg/\left(m}^{2}·s\right)$$
</div>
This value is the same as the value given for the acoustic impedance of fat tissue.

**Strategy for (b)**

The intensity reflection coefficient for any boundary between two media is given by $$a=\frac{ {\left({Z}_{2}-{Z}_{1}\right)}^{2}}{ {\left({Z}_{1}\phantom{\rule{0.25em}{0ex}}+\phantom{\rule{0.25em}{0ex}}{Z}_{2}\right)}^{2}}$$

, and the acoustic impedance of muscle is given in [\[link\]](#import-auto-id2672118).

**Solution for (b)**

Substitute known values into $$a=\frac{ {\left({Z}_{2}-{Z}_{1}\right)}^{2}}{ {\left({Z}_{1}\phantom{\rule{0.25em}{0ex}}+\phantom{\rule{0.25em}{0ex}}{Z}_{2}\right)}^{2}}$$

 to find the intensity reflection coefficient:

<div data-type="equation" id="eip-232">
$$a=\frac{ {\left({Z}_{2}-{Z}_{1}\right)}^{2}}{ {\left({Z}_{1}+{Z}_{2}\right)}^{2}}=\frac{ {\left(1\text{.}\text{34}×{\text{10}}^{6}\phantom{\rule{0.25em}{0ex}}{\text{kg/(m}}^{\text{2}}\mathrm{· s\right)}-1.70×{\text{10}}^{6}\phantom{\rule{0.25em}{0ex}}{\text{kg/(m}}^{\text{2}}· s\right)\right)}^{2}}{ {\left(1\text{.}\text{70}×{\text{10}}^{6}\phantom{\rule{0.25em}{0ex}}{\text{kg/(m}}^{\text{2}}· s\right)+1\text{.}\text{34}×{\text{10}}^{6}\phantom{\rule{0.25em}{0ex}}{\text{kg/(m}}^{\text{2}}· s\right)\right)}^{2}}=0\text{.}\text{014}$$
</div>
**Discussion**
...
The applications of ultrasound in medical diagnostics have produced untold benefits with no known risks. Diagnostic intensities are too low (about $${\text{10}}^{-2}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
...
. Indeed, current technology cannot do quite this well. Abdominal scans may use a 7-MHz frequency, and the speed of sound in tissue is about 1540 m/s—so the wavelength limit to detail would be $$\lambda =\frac{ {v}_{w}}{f}=\frac{\text{1540 m/s}}{7×{\text{10}}^{6}\phantom{\rule{0.25em}{0ex}}\text{Hz}}=0\text{.}\text{22 mm}$$
...
. In practice, 1-mm detail is attainable, which is sufficient for many purposes. Higher-frequency ultrasound would allow greater detail, but it does not penetrate as well as lower frequencies do. The accepted rule of thumb is that you can effectively scan to a depth of about $$500\lambda $$
 ...
 into tissue. For 7 MHz, this penetration limit is $$500×0.22 mm$$
...
The first two questions can be answered using $${f}_{\text{obs}}={f}_{s}\left(\frac{ {v}_{w}}{ {v}_{w}\phantom{\rule{0.25em}{0ex}}±\phantom{\rule{0.25em}{0ex}}{v}_{s}}\right)$$

 and $${f}_{\text{obs}}={f}_{s}\left(\frac{ {v}_{w}\phantom{\rule{0.25em}{0ex}}±\phantom{\rule{0.25em}{0ex}}{v}_{\text{obs}}}{ {v}_{w}}\right)$$
...
$${f}_{\text{obs}}=\left(2,\text{500},\text{000 Hz}\right)\left(\frac{\text{1540 m/s}+0\text{.}\text{2 m/s}}{\text{1540 m/s}}\right)$$
</div>
...
* The frequency leaving the blood is 2,500,325 Hz, but it is shifted upward as given by
  ...
  $${f}_{\text{obs}}={f}_{s}\left(\frac{ {v}_{w}}{ {v}_{w}\phantom{\rule{0.25em}{0ex}}–\phantom{\rule{0.25em}{0ex}}{v}_{b}}\right).$$
  </div>
...
$${f}_{\text{obs}}=\left(2,\text{500},\text{325 Hz}\right)\left(\frac{\text{1540 m/s}}{\text{1540 m/s}-0\text{.}\text{200 m/s}}\right)$$
</div>
...
$$\mid 2,\text{500},\text{649}\phantom{\rule{0.25em}{0ex}}\text{Hz}-2,\text{500},\text{000}\phantom{\rule{0.25em}{0ex}}\text{Hz}\mid $$
</div>
...
<div data-type="exercise" data-element-type="conceptual-questions">
<div data-type="problem" markdown="1">
Suppose you read that 210-dB ultrasound is being used to pulverize cancerous tumors. You calculate the intensity in watts per centimeter squared and find it is unreasonably high ($${\text{10}}^{5}\phantom{\rule{0.25em}{0ex}}{\text{W/cm}}^{2}$$
...
<div data-type="problem" markdown="1">
What is the sound intensity level in decibels of ultrasound of intensity $${\text{10}}^{5}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}$$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Find the sound intensity level in decibels of $$2.00×{\text{10}}^{–2}\phantom{\rule{0.25em}{0ex}}{\text{W/m}}^{2}\phantom{\rule{0.25em}{0ex}}$$
...
<div data-type="solution" markdown="1">
(a) $$5.78×{10}^{–4}\phantom{\rule{0.25em}{0ex}}\text{m}$$

(b) $$2.67×{10}^{6}\phantom{\rule{0.25em}{0ex}}\text{Hz}$$
...
<div data-type="solution" markdown="1">
(a) $${v}_{w}=1540 m/s=\mathrm{f\lambda }⇒\lambda =\frac{1540 m/s}{100×{10}^{3}\phantom{\rule{0.25em}{0ex}}\text{Hz}}=0.0154 m < 3.50 m.$$
