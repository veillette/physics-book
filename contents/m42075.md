...
In this section we have introduced the quantity normal force, which is represented by the variable  $$ \mathbf{\text{N}} $$ . This should not be confused with the symbol for the newton, which is also
...
where $$ F_{\text{net}\parallel }=w_{\parallel }=mg \sin\left(\text{25º}\right) $$ 

, assuming no friction for this part, so that

<div data-type="equation" id="eip-id1172560">
 $$ a_{\parallel }=\frac{ F_{\text{net}\parallel }}{m}=\frac{mg \sin\left(\text{25º}\right)}{m}=g\sin\left(\text{25º}\right) $$ 
</div>
...
<div data-type="equation" id="eip-id1527388">
 $$ a_{\parallel }=\frac{ F_{\text{net}\mid \mid }}{m}=\frac{ w_{\parallel }-f}{m}=\frac{mg \sin\left(\text{25º}\right)-f}{m}. $$ 
</div>
...
<div data-type="equation" id="eip-id2503179">
 $$ \begin{array}{lll}\cos\left(\theta \right)& =& \frac{ w_{\perp }}{w}\\ w_{\perp }& =& w\cos\left(\theta \right)=mg \cos\left(\theta \right)\end{array} $$ 
</div>
<div data-type="equation" id="eip-id1900825">
 $$ \begin{array}{lll}\sin\left(\theta \right)& =& \frac{ w_{\parallel }}{w}\\ w_{\parallel }& =& w\sin\left(\theta \right)=mg \sin\left(\theta \right)\end{array} $$ 
</div>
</div>
...
<div data-type="equation" id="eip-id2489897">
 $$ T=mg =\left(5.00 \text{kg}\right)\left(9.80 \text{m/s}^2\right)=49.0 \text{N}. $$ 
</div>
...
<div data-type="equation" id="eip-id2089830">
 $$ T=\frac{w}{2\sin\left(5.0º\right)}=\frac{mg }{2\sin\left(5.0º\right)}, $$ 
</div>
so that
...
* When objects rest on an inclined plane that makes an angle $$ \theta $$
  ...
  <div data-type="equation" id="eip-id2419526">
   $$ w_{\parallel }=w\text{sin}\left(\theta \right)=mg \text{sin}\left(\theta \right) $$ 
  </div>
