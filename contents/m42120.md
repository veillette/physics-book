...
of **significant figures**{: data-type="term" #import-auto-id2992464}, the rule is that **the last digit written down in
...
circle can be calculated from its radius using $$ A=\pi r^{2} $$. 
Let us see how many significant figures the area has if the radius has only
two—say, $$ r=1.2\text{m} $$. Then,

<div data-type="equation" id="eip-722">
 $$ A=\pi r^{2}=\left(3.1415927 \dots \right) \times  \left(1.2\text{m} \right)^{2}
=4.5238934\text{m}^{2} $$ 
</div>
...
If $$ 40 \pm  1 $$ beats are counted in $$ 30.0 \pm 0.5\text{s} $$, what is the heart rate and its uncertainty in beats per minute?
