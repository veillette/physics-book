...
The energy given to the proton in each gap between tubes is  $${\text{PE}}_{\text{elec}}=\text{qV} $$
 ...
 is the potential difference (voltage) across the gap. Since  $$q={q}_{e}=1.6×{\text{10}}^{-19} \text{C} $$
 and  $$\text{1 eV}=\left(\text{1 V}\right)\left(1.6×{\text{10}}^{-\text{19}} \text{C}\right) $$
...
(a)  $$2\text{.}\text{09}×{\text{10}}^{-5} \text{s} $$
(b)  $$4\text{.}\text{77}×{\text{10}}^{4} \text{Hz} $$
</div>
</div>
...
<div data-type="problem" markdown="1">
Suppose a  $${W}^{-} $$
 created in a bubble chamber lives for  $$5\text{.}\text{00}×{\text{10}}^{-\text{25}} \text{s}. $$
...
What length track does a  $${\pi }^{+} $$
 traveling at 0.100 *c* leave in a bubble chamber if it is created there and lives for  $$2\text{.}\text{60}×{\text{10}}^{-8} \text{s} $$
