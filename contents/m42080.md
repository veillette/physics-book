...
equation can also be written in a more generalized fashion as$$ F_{\text{D}}=b v^{2}  $$, where $$ b $$
** is a constant equivalent to $$ 0.5 C \rho A $$. We have set the exponent for these equations as 2 because, when an object is
...
$$ F_{\text{D}}=\frac{1}{2} C\rho Av^{2}\text{,} $$ 
</div>
where  $$ C $$ 
...
is proportional to the speed, a heavier skydiver must go faster for$$ F_{\text{D}} $$
...
<div data-type="equation" id="eip-981">
 $$ \begin{array}{lll}v& =& \sqrt{\frac{2\left(75 \text{kg}\right)\left(9.80 \text{m/s}^{2}\right)}{\left(1.21 \text{kg}{\text{/m}}^{3}\right)\left(0.70\right)\left(0.18\text{m}^{2}\right)}}\\ & =& 98 \text{m/s}\\ & =& 350 \text{km/h}\text{.}\end{array} $$ 
</div>
