...
Assume the density of air is $$ \rho =1.21 \text{kg/m}^{3} $$. A 75-kg skydiver descending head first will have an area approximately $$
A=0.18\text{m}^{2} $$
...
=& 44 \text{m/s.}\end{array} $$ 
</div>
**Discussion**

This result is consistent with the value for $$ v_{\text{t}} $$
mentioned earlier. The 75-kg skydiver going feet first had a $$ v=98 \text{m/s} $$.
He weighed less but had a smaller frontal area and so a smaller drag due to
the air.
...
) can be about $$ 2  \text{μm/s} $$. To move at a greater speed, many bacteria swim using flagella (organelles
