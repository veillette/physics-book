...
where $$ \mu_{\text{s}} $$ is the coefficient of static friction and $$ N $$ is
...
friction can have a minimum and a maximum value of $$ \mu_{\text{s}}N $$. Static
...
9.80\text{m/s}^{2}\right)=980 \text{ N} $$, perpendicular to the floor. If the
coefficient of static friction is 0.45, you would have to exert a force parallel
to the floor greater than $$ f_{\text{s}\left(\text{max}\right)}=\mu_
{\text{s}}N=\left(0.45\right)\left(
980 \text{N}\right)
=440\text{N} $$ to move the crate. Once there is motion, friction is less and
the coefficient of kinetic friction might be 0.30, so that a force of only 290
N ( $$ f_{\text {k}}=\mu_{\text{k}}N=\left(0.30\right)\left( 980\text{N}\right)
=290\text{N} $$ ) would keep it moving at a constant speed. If the floor is
...
* {: #import-auto-id1165296328554} Friction is a contact force between systems
  ...
  friction $$ f_{\text{s}} $$ between systems stationary relative to one another
  is given by
  <div data-type="equation" id="eip-940">
   $$ f_{\text{s}}\le \mu_{\text{s}}N, $$ 
  </div>

  where $$ \mu_{\text{s}} $$ is the coefficient of static friction, which
...
friction {: #import-auto-id1165298541210}
