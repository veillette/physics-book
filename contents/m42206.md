...
<div data-type="equation" id="eip-579">
 $$\begin{array}{lll}{P}_{1}& =&  1.01 \times 10^{5}  {\text{N/m}}^{2}\\ & & \text{}+\frac{1}{2}\left({\text{10}}^{3} {\text{kg/m}}^{3}\right)\left[\left(\text{25.5 m/s}{\right)}^{2}-\left(\text{1.96 m/s}{\right)}^{2}\right]\\ & =&  4.24 \times 10^{5}  {\text{N/m}}^{2}\text{.}\end{array} $$
</div>
**Discussion**
