...
example) produced by the force $$ F $$
**, and $$ k $$ is a proportionality constant that depends on the shape and
...
original length. [\[link\]](#import-auto-id1165298671576) lists values of <em>
$$ Y$ </em> for several materials—those with a large $Y $$ are said to have a
...
The force is equal to the maximum tension, or $$ F=3.0\times 10^{6}\text{N} $$.
The cross-sectional area is $$ \pi r^{2}=2.46 \times 10^{-3}\text{m}^{2} $$. The
...
), and the ratio of the change in length to length, $$ \frac{\Delta L}{ L_{0}}
...
can be described with similar equations. The expression for **shear
deformation**{: data-type="term" #import-auto-id1165296259204} is
...
$$ 1.80 \text{ µm} $$ —an amount undetectable to the unaided eye.
...
, $$ V_{0} $$ is the original volume, and * $$ \frac{F}{A} $$
...
* {: #import-auto-id1165298476009} Hooke’s law is given by
  ...
  *{::}** is *Young’s modulus*, which depends on the substance, * $$ A $$
    * is the cross-sectional area, and $$ L_{0} $$ is the original length.
...
diameter, and can be considered to have half the stiffness of hardwood. (a)
Calculate the compression of the pole. (b) Find how much it bends and in what
direction. (c)
