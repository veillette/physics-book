...
Thus, when the girl throws the snowball forward,  $$u=1.0 \text{m/s}+1.5 \text{m/s}=2.5 \text{m/s} $$ .
 It makes good intuitive sense that the snowball will head towards the Earth-bound observer faster, because it is thrown forward from a moving vehicle. When the girl throws the snowball backward,  $$u=1.0 \text{m/s}+\left(-1.5 \text{m/s}\right)=-0.5 \text{m/s} $$ .
