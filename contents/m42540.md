...
<div data-type="solution" markdown="1">
 ...
 $$\begin{array}{ll}u& =& \frac{v+u^\prime }{1+\left( v u^\prime /{c}^{2}\right)}=\frac{v+c}{1+\left(\text{vc}/{c}^{2}\right)}=\frac{v+c}{1+\left(v/c\right)}\\ & =& \frac{c\left(v+c\right)}{c+v}=c\end{array} $$
</div>
</div>
