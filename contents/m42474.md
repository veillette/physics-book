...
$$\frac{1}{ {d}_{\text{o}}}=\frac{1}{0\text{.}\text{250 m}}-\frac{1}{3\text{.}\text{00 m}}=\frac{3\text{.}\text{667}}{\text{m}}\text{.}$$
</div>
...
$${d}_{\text{o}}=\frac{\text{1 m}}{3\text{.}\text{667}}=\text{27}\text{.}3 cm.$$
</div>
**Discussion**
...
The insolation is $$\text{900 W}{\text{/m}}^{2}$$

. We must find the cross-sectional area $$A$$

 of the concave mirror, since the power delivered is $$\text{900 W}{\text{/m}}^{2}×\text{A}$$

. The mirror in this case is a quarter-section of a cylinder, so the area for a length $$L$$

 of the mirror is $$A=\frac{1}{4}\left(2\mathrm{\pi R}\right)L$$

. The area for a length of 1.00 m is then

<div data-type="equation" id="eip-48">
$$A=\frac{\pi }{2}R\left(1.00 m\right)=\frac{\left(3.14\right)}{2}\left(0.800 m\right)\left(1.00 m\right)=1.26\phantom{\rule{0.25em}{0ex}}{m}^{2}.$$
</div>
The insolation on the 1.00-m length of pipe is then

<div data-type="equation" id="eip-667">
$$\left(9.00×{10}^{2}\frac{W}{ {m}^{2}}\right)\left(1.26\phantom{\rule{0.25em}{0ex}}{m}^{2}\right)=1130 W.$$
</div>
**Solution to (c)**

The increase in temperature is given by $$Q=\mathrm{mc}\Delta T$$
...
$$\begin{array}{lll}m& =& \rho \text{V}=\mathrm{\rho \pi }{\left(\frac{d}{2}\right)}^{2}\left(1.00 m\right)\\ & =& \left(\text{8.00}×{10}^{2}\phantom{\rule{0.25em}{0ex}}{\text{kg/m}}^{3}\right)\text{(3.14)}{\text{(0.0100 m)}}^{2}\left(1.00 m\right)\\ & =& 0.251 kg.\end{array}$$
</div>
...
An array of such pipes in the California desert can provide a thermal output of 250 MW on a sunny day, with fluids reaching temperatures as high as $$\text{400º}\text{C}$$
...
$${d}_{\text{i}}=-{\text{md}}_{\text{o}}\text{.}$$
</div>
Entering known values yields

<div data-type="equation" id="eip-372">
$${d}_{\text{i}}=–\left(0\text{.}\text{0320}\right)\left(\text{12.0 cm}\right)=\text{–0.384 cm.}$$
</div>
...
$$\frac{1}{f}=\frac{1}{\text{12.0 cm}}+\frac{1}{-0\text{.}\text{384 cm}}=\frac{-2\text{.}\text{52}}{\text{cm}}\text{.}$$
</div>
...
<div data-type="solution" markdown="1">
(a) $$–1.5×{10}^{–2}\phantom{\rule{0.25em}{0ex}}\text{m}$$
...
<div data-type="exercise" data-element-type="problems-exercises">
<div data-type="problem" markdown="1">
Ray tracing for a flat mirror shows that the image is located a distance behind the mirror equal to the distance of the object from the mirror. This is stated $${d}_{\text{i}}={\mathrm{–d}}_{\text{o}}$$
...
Referring to the electric room heater considered in the first example in this section, calculate the intensity of IR radiation in $${\text{W/m}}^{2}$$
 ...
 projected by the concave mirror on a person 3.00 m away. Assume that the heating element radiates 1500 W and has an area of $$\text{100}\phantom{\rule{0.25em}{0ex}}{\text{cm}}^{2}$$
