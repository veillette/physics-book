...
$$\left(9.00×{10}^{2}\frac{W}{ {m}^{2}}\right)\left(1.26 {m}^{2}\right)=1130 W.$$
</div>
**Solution to (c)**
...
$$\begin{array}{lll}m& =& \rho \text{V}=\mathrm{\rho \pi }{\left(\frac{d}{2}\right)}^{2}\left(1.00 m\right)\\ & =& \left(\text{8.00}×{10}^{2} {\text{kg/m}}^{3}\right)\text{(3.14)}{\text{(0.0100 m)}}^{2}\left(1.00 m\right)\\ & =& 0.251 kg.\end{array}$$
</div>
...
Referring to the electric room heater considered in the first example in this section, calculate the intensity of IR radiation in $${\text{W/m}}^{2}$$
 projected by the concave mirror on a person 3.00 m away. Assume that the heating element radiates 1500 W and has an area of $$\text{100} {\text{cm}}^{2}$$
