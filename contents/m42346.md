...
[\[link\]](#Table1) gives representative values of  $$\rho  $$ .
...
where  $${\rho }_{0} $$
 ...
 in [\[link\]](#Table2) below.) For larger temperature changes,  $$\alpha  $$
...
** close to zero (to three digits on the scale in [\[link\]](#Table2)), and so its resistivity varies only slightly with temperature. This is useful for making a temperature-independent resistance standard, for example.
...
*  Values of
   $$\rho  $$ in [\[link\]](#Table1) show that materials fall into three groups—*conductors, semiconductors, and insulators*.
...
In which of the three semiconducting materials listed in [[link]](#Table1) do impurities supply free charges? (Hint: Examine the range of resistivity for each and determine whether the pure semiconductor has the higher or lower conductivity.)
