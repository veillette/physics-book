...
class MathDelimiterFixer {
  ...
  fixFileContent(content, filename) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (trimmed === '$$' && prevMatches && prevMatches.length === 1) {
    ...
    if (result.modified) {
      ...
      if (stillHasErrors.length < errorLines.length) {
        ...
        if (applyFixes) {
          ...
          console.log(
            chalk.green(
              `  ✓ Fixed ${filename} (${result.fixes.length} fixes, ${errorLines.length - stillHasErrors.length} delimiters resolved)`
            )
          );
        } else {
          console.log(
            chalk.blue(
              `  → Would fix ${filename} (${result.fixes.length} fixes, ${errorLines.length - stillHasErrors.length} delimiters resolved)`
            )
          );
        }
        ...
        if (verbose) {
          result.fixes.forEach((fix, idx) => {
            console.log(
              chalk.gray(`    ${idx + 1}. ${fix.type} (lines ${fix.lineStart}-${fix.lineEnd})`)
            );
          });
        }
        ...
        return {
          ...
          fixed: true,
          fixes: result.fixes,
          errorsRemaining: stillHasErrors.length,
        };
      } else {
        console.log(
          chalk.red(`  ✗ ${filename}: Fixes didn't help (still ${stillHasErrors.length} errors)`)
        );
  ...
  processAllFiles() {
    ...
    console.log(
      chalk.gray(`Mode: ${applyFixes ? 'APPLY FIXES' : 'DRY RUN (use --apply to fix)'}\n`)
    );
